...
|---|---|---|---|
| `bulletHitEffect` | `src/shell/bullets.js:144` | `{ x, y, z, dx, dy, dz, player: this.player }` | A bullet impact occurred (wall or player) — a visual/effect hook, fires on both client and server. |
| `bulletHitAfter` | `src/shell/bullets.js:163` | `{ x, y, z, dx, dy, dz, player: this.player }` | After hit resolution/damage has been applied for that bullet. |
...
| `GameTypesInit` | `src/shell/gametypes.js:100` | `{ GameTypes, ItemTypes, defaultOptions }` | Before `GameTypes` is deep-merged against `defaultOptions` — the extension point for registering an entirely new gamemode. |
...
| `permissionsAfterSetup` | `src/shell/permissions.js:599` | `{ this: this }` | End of `PermissionsConstructor`'s constructor, after every built-in command is registered — the extension point for registering new slash commands via `data.this.newCommand(...)`. |
| `updateBefore` | `src/shell/player.js:233` | `{ player: this, delta, resim }` | Start of `Player.update()`, before movement/physics for that tick — the main per-player-per-tick hook. |
| `onStandOnBlock` | `src/shell/player.js:738` | `{mesh, this: this}` | Per-tick, when a player's feet resolve against a block mesh — used for special surface behavior (e.g. a parkour gamemode). |
| `onStandOnTile` | `src/shell/player.js:741` | `{mesh, this: this}` | Per-tick, when a player's feet resolve against a tile mesh — same use case as `onStandOnBlock` for a different mesh category. |
| `canJump` | `src/shell/player.js:750` | `{ this: this, canJump }` | Before jump eligibility is finalized — a listener can override the boolean. |
| `onPlayerDeath` | `src/shell/player.js:1196` | `{player: this, firedId}` | A player has died — `firedId` identifies the killer (or is absent/null for an environmental death). |
| `prepareBabylonBefore` | `src/shell/general/prepare-babylons.js:44` | `{ baseBabylons, babylonDirFiles, addBabylonToZip }` | Model-build pipeline: register new `.babylon` filenames into `data.baseBabylons` before file paths are resolved. |
| `prepareBabylon` | `src/shell/general/prepare-babylons.js:64` | `{ filename, baseBabylon, extraBabylons }` | Model-build pipeline: supply the actual file path for a previously-registered `.babylon` filename. |
| `prepareBabylonExtra` | `src/shell/general/prepare-babylons.js:86` | `{ filename, baseBabylon, extraBabylonData, item }` | Model-build pipeline: reconcile a per-item "extra" babylon/skeleton against the base model (e.g. bone-index fixups). |
