...
- **Server-side**: on `game:roomInitEnd`, if the "christmas" event is currently active (via `#events`' `getEventsAtTime()`) and the map doesn't opt out (`minMap.extents.seasonalEffectsDisabled`), sets `gameOptions.plugins.weather.snowStormEnabled = true` for that room (weather state lives under the `legacyweather` plugin's own `gameOptions.plugins.weather`, not a top-level `gameOptions.weather`).
