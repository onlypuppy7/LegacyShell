...
const exported = {
    createSession: async (user_id, ip_address) => {
        ss.config.verbose && log.info("reason for session deletion: new created "+user_id);
        ...
        try {
            ss.config.verbose && log.bgBlue("services: Writing to DB: create new session "+user_id+", "+session_id);
            await ss.runQuery(`
    ...
    retrieveSession: async (session_id, ip_address, readOnly) => {
        try {
            ss.config.verbose && log.bgCyan("services: Reading from DB: get session for session_id " + session_id);
            ...
            if (!session) {
                log.yellow(`No valid session found for session_id: ${session_id}`);
                return null;
            };
            ...
            if (session.ip_address !== ip_address && !readOnly) {
                ss.config.verbose && log.info("reason for session deletion: ip mismatch "+session_id);
                await exported.deleteAllSessionsForUser(session.user_id);
                return null;
            };
    
            log.green(`Session retrieved for session_id: ${session_id}`);
            return session;
        } catch (error) {
    ...
    cleanupExpiredSessions: async () => {
        try {
            ss.config.verbose && log.bgPurple(`services: Deleting from DB: Cleaning up expired sessions`);
            await ss.runQuery(`DELETE FROM sessions WHERE expires_at < strftime('%s', 'now')`);
            log.green('Expired sessions cleaned up');
        } catch (error) {
