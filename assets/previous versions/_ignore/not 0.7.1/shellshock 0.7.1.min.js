...
function respawn() {
    if (queueVideoAd) respawnTime < 0 && playVideoAd();
    else if (respawnTime < 0) {
        document.getElementById("DEATH_BOX").style.display = "none", document.getElementById("aipGameContainer").style.display = "none", document.getElementById("respawnButton").style.display = "none", document.pointerLockElement || canvas.requestPointerLock(), addCanvasListeners();
        var e = new Comm.output(1);
        e.packInt8(Comm.requestRespawn), ws.send(e.buffer)
    }
}
...
function startRendering() {
    document.getElementById("mainMenu").style.display = "none", document.getElementById("overlay").style.display = "none", gameType == GameType.teams && (document.getElementById("switchTeamButton").style.visibility = "visible"), document.getElementById("homeButton").style.visibility = "visible", document.getElementById("friendsButton").style.visibility = "visible", document.getElementById("game").style.display = "block", document.getElementById("gameCanvasContainer").appendChild(canvas), resize(), captureKeys(), closeAlertDialog(), scene.registerBeforeRender(function () {
    ...
    ws.onmessage = function (e) {
        for (var t = new Comm.input(e.data); t.isMoreDataAvailable();) switch (t.unPackInt8U()) {
            ...
            case Comm.addPlayer:
                ...
                players[i.id] || (meId == i.id || (i.name = i.name.replace(/</g, ""), 0 == i.name.length ? i.name = "Anonymous" : (i.name = fixStringWidth(i.name), isBadWord(i.name) && (i.name = "!@#$"))), addPlayer(i));
                break;
            ...
            case Comm.die:
                ...
                if (c != meId) {
                    ...
                    deaths++, camera.parent = null, camera.position = new BABYLON.Vector3(me.actor.mesh.position.x, me.actor.mesh.position.y + .2, me.actor.mesh.position.z), m && (camera.lockedTarget = m.actor.bodyMesh), removeCanvasListeners(), respawnTime = z, isTimeToPlayVideoAd() && (queueVideoAd = !0, respawnTime = 2);
                    ...
                    var S = setInterval(function () {
                        k.innerText = "You may respawn in " + respawnTime, --respawnTime < 0 && (clearInterval(S), k.innerText = "", document.getElementById("respawnButton").style.display = "block")
                    }, 1e3);
                    setTimeout(function () {
                        document.exitPointerLock()
            ...
            case Comm.respawn:
                var r = t.unPackInt8U(),
                    ...
                    d = t.unPackFloat(),
                    ie = t.unPackInt8U();
                (C = players[r]) && (C.respawn(n, o, d, ie), r == meId && (camera.position = BABYLON.Vector3.Zero(), camera.rotation = BABYLON.Vector3.Zero(), camera.rotationQuaternion = BABYLON.Quaternion.Zero(), camera.parent = me.actor.eye, camera.lockedTarget = null));
                break;
...
function Scope() {
    this.crosshairs = new BABYLON.AbstractMesh("", scene), this.crosshairs.setEnabled(!1), this.crosshairs.position.z = 2;
...
function addCanvasListeners() {
    canvas.style.pointerEvents = "all", canvas.addEventListener("click", onCanvasClick, !1), canvas.addEventListener("mousedown", onCanvasMouseDown, !1), canvas.addEventListener("mouseup", onCanvasMouseUp, !1), canvas.addEventListener("mousemove", onCanvasMouseMove)
}

function removeCanvasListeners() {
    canvas.style.pointerEvents = "none", canvas.removeEventListener("click", onCanvasClick), canvas.removeEventListener("mousedown", onCanvasMouseDown), canvas.removeEventListener("mouseup", onCanvasMouseUp), canvas.removeEventListener("mousemove", onCanvasMouseMove)
}
...
function onResourcesLoaded() {
    function e() {
        document.getElementById("className").innerText = classes[selectedClass].name, document.getElementById("classWeapon").innerText = "Weapon: " + a[selectedClass].weapon.name;
        for (y in d) {
            var e = (a[selectedClass].weapon[y] + d[y].min) / (d[y].max - Math.abs(d[y].min));
            e = Math.floor(95 * e) + 5, document.getElementById(d[y].name).style.width = e + "%"
        }
    }
    ...
    r.position.y = -.32, r.rotation.x = Math.PI / 2, r.receiveShadows = !0, r.material = i, light.position.y = 2, selectedColor = getStoredNumber("selectedColor", 0), selectedClass = getStoredNumber("selectedClass", Math.randomInt(0, classes.length));
    for (var a = [], n = 0; n < t; n++) {
        var o = new function (e, t, i, r) {
            this.id = 1, this.name = "", this.x = e, this.y = t, this.z = i, this.hp = 100, this.shellColor = 0, this.actor = new PlayerActor(this), this.actor.mesh.setPivotPoint(new BABYLON.Vector3(0, 0, -1.6), BABYLON.Space.GLOBAL), this.actor.head.rotation.y = 2.5, this.weapon = new classes[r].weapon(this), this.weapon.actor.equip()
        }(0, -.32, 1.6, n);
        o.actor.mesh.rotation.y = n * Math.PI / (t / 2), a.push(o)
    }
    for (var d = {
            totalDamage: {
                name: "damage",
        ...
        var s = a[n].weapon;
        for (var y in d) d[y].flip ? (d[y].max = Math.max(d[y].max, -s[y]), d[y].min = Math.min(d[y].min, -s[y])) : (d[y].max = Math.max(d[y].max, s[y]), d[y].min = 0)
    }
    var l = -selectedClass;
    window.selectClass = function (i) {
        selectedClass = Math.mod(selectedClass + i, t), localStorage.setItem("selectedClass", selectedClass), l -= i, e()
    }, window.selectColor = function (e) {
        selectedColor = e.id.substr(5), a[selectedClass].actor.setShellColor(selectedColor), localStorage.setItem("selectedColor", selectedColor)
    }, e(), BABYLON.Engine.audioEngine.setGlobalVolume(settings.volume), engine.runRenderLoop(function () {
        scene.render();
        var e = l / 5;
        l -= e;
        for (var i = 0; i < t; i++) a[i].actor.mesh.rotation.y += e * (Math.PI / (t / 2))
    }), bigAdShown = !1
}
...
function play(e) {
    ...
    0 == t && (t = Date.now(), localStorage.setItem("lastPreRoll", t)), adTest || Date.now() > t + 3e5 && timesPlayed % 2 == 1 ? (document.getElementById("aipMenuContainer").style.display = "none", playVideoAd()) : login()
}

function login() {
    if (BABYLON.Engine.audioEngine.audioContext.resume && BABYLON.Engine.audioEngine.audioContext.resume(), 0 != (username = document.getElementById("username").value.trim()).length) {
...
function showBigAd() {
    document.getElementById("bigAd").style.display = "block", document.getElementById("bigAdImg").src = "img/ads/f13_shellshockers_preLaunch.jpg", ga("send", "event", {
...
function showMainMenu() {
    if (openAlertDialog("LOADING", "Just a moment!", null, null, !0), document.getElementById("switchTeamButton").style.visibility = "hidden", document.getElementById("homeButton").style.visibility = "hidden", document.getElementById("friendsButton").style.visibility = "hidden", document.getElementById("game").style.display = "none", document.getElementById("inviteFriends").style.display = "none", resetGameUI(), kills = 0, deaths = 0, bestKillStreak = 0, document.getElementById("clickBlocker").style.display = "none", document.body.style.overflow = "visible", inGame && (document.getElementById("aipGameContainer").style.display = "none", document.getElementById("aipMenuContainer").style.display = "block", document.getElementById("aipMenuContainer").appendChild(aipBanner), document.getElementById("respawnButton").style.display = "none", aiptag.cmd.display.push(function () {
            aipDisplayTag.refresh("shellshock-io_300x250")
...
function playVideoAd() {
    var e;
    switch (e = inGame && deaths > 1 ? 0 : Date.now() % 2, lastAdProvider = e, queueVideoAd = !1, e) {
        case 0:
            document.getElementById("overlay").style.display = "block", aiptag.cmd.player.push(function () {
        ...
        case 1:
            ...
            r.src = "//cdn.playwire.com/bolt/js/zeus/embed.js", r.type = "text/javascript", r.setAttribute("charset", "utf-8"), r.setAttribute("data-config", "//config.playwire.com/1022547/v2/pre_content.json"), r.setAttribute("data-width", "640px"), r.setAttribute("data-height", "360px"), r.setAttribute("data-id", "pre-content-player"), r.setAttribute("data-hidden-container", "dummyContent"), r.setAttribute("data-onready", "window.boltEventHandlers"), t.style.display = "block", i.appendChild(r), localStorage.setItem("lastPreRoll", Date.now());
            var a = setInterval(function () {
                if (void 0 !== typeof Bolt) {
                    var e = window.Bolt._adBlockDetected;
                    console.log("hasAdBlock ?", e), console.log("bolt done"), boltPlayerDone(), clearInterval(a)
                }
            }, 1e3)
    }
}
...
function boltPlayerDone() {
    ...
    e.parentNode.removeChild(e), document.getElementById("playwireModal").style.display = "none", inGame || login()
}
...
function isBadWord(e) {
    var t = (e = " " + e + " ").toLowerCase().replace(/[^a-zA-Z0-9|!\|@|$| ]/g, "").replace(/6|g/g, "9").replace(/b/g, "6").replace(/\||l|i|1|;|Â¡/g, "!").replace(/e/g, "3").replace(/a|@/g, "4").replace(/o/g, "0").replace(/s|\$/g, "5").replace(/t/g, "7").replace(/z/g, "2").replace(/7h3|my|y0ur|7h3!r|h!5|h3r/g, ""),
    ...
    window.location.hash && (openJoinBox(), document.getElementById("joinCode").value = window.location.hash.substr(1)), aipBanner = document.getElementById("aipBanner"), aiptag.cmd.display.push(function () {
        ...
        adplayer = new aipPlayer({
            ...
            AIP_COMPLETE: function () {
                document.getElementById("overlay").style.display = "none", inGame || login()
            },
            AIP_REMOVE: function () {
...
PlayerActor.prototype.drawTextOnNameTexture = function (e, t, i, r, a, n) {
    ...
    if (this.player.id != meId) {
        ...
        m != this.mesh.isVisible && (this.mesh.setVisible(m), m ? this.showNameSprite() : this.hideNameSprite()), this.nameSprite && (this.nameSprite.position.x = this.mesh.position.x, this.nameSprite.position.y = this.mesh.position.y + .5 * this.nameSprite.height + .65, this.nameSprite.position.z = this.mesh.position.z)
    }
