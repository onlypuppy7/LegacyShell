    ...
    while (input.isMoreDataAvailable()) {
        ...
        switch (cmd) {
            ...
            case Comm.Code.chat:
                ...
                player.muted || (chatParser.innerHTML = text, text = chatParser.textContent.trim(), player && 0 < text.length && !censor.detect(text) && text.indexOf("<") < 0 && addChat(text, player));
                break;
            case Comm.Code.jump:
                var id = input.unPackInt8U(),
                    player = players[id];
...
function loadMapMeshes(scene, onComplete) { //8th, loads map meshes, wowie (name from deobf leak)
    mapMeshes = [null];
    var mat;
...
function generateLightmapAndSkybox(This, complete) { //11th, creates the lightmap (offloaded onto a worker) and skybox
...
function onMapComplete() { //12th,  (callback of loadMap at end of onLoadingComplete)
...
function startRendering() { //13th, starts rendering and game loop
    hideDarkOverlay();
...
function mainGameLoop () { //14th, this is the final-ish stage of the sequence.
    (function () {
...
const censor = { //old isBadWord function
    homoglyphs: [
        [["6", "g"], "9"],
        [["b"], "6"],
        [["|", "l", "i", "1", ";", "¡"], "!"],
