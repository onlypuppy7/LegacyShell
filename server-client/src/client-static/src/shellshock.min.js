            ...
            case Comm.Code.switchTeam:
                ...
                if (player) {
                    ...
                    for (var i = 0; i < playerLimit; i++){
                        player = players[i];
                        if (player.actor) player.actor.updateTeam();
                    };
                    rebuildPlayerList()
                };
                break;
...
function loadSounds(scene, onComplete) { //6th, loads sounds (who knew?)
    var options = {
        spatialSound: true,
        distanceModel: "linear", // You can try "exponential" as well
        rolloffFactor: 2,        // Increase for quicker attenuation
        maxDistance: 100,        // Set a maximum distance
        refDistance: 1           // Distance at which sound level is half the max volume
    };
    var loadsComplete = 0;
...
function rebuildPlayerList() { //by player list we mean the literal html element. pretty sure this is purely just for displaying the player list (the one on the left side)
    ...
    for (i = 0; i < playerIdxs.length; i++) {
        ...
        let pingIcon = list[i].querySelector("img"); // Try to find the existing image element
