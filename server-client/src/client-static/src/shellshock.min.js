        ...
        switch (cmd) {
            ...
            case Comm.Code.chat:
                ...
                if (!player.muted) {
                    ...
                    if (player && 0 < text.length && !censor.detect(text, settings.enable18Chat) && text.indexOf("<") < 0) {
                        addChat(text, player);
                    };
                };
                break;
...
function addChat(str, fromPlayer) {
    ...
    str = '<div class="shadedPane" style="margin-top: 0.2em">' + str + "</div><br>";
...
window.extern = function () {
    return {
        ...
        onChatKeyDown: function (e) {
            ...
            // devlog("onChatKeyDown", key);
            var text = chatInEl.value.trim();
            switch (key) {
                case "ArrowUp":
                    if (previousChatsIdx < previousChats.length) {
                ...
                case "Enter":
                    if ("" != text) {
                        ...
                        previousChatsIdx = -1;
                        me.chatLineCap--;
                        if (!text.startsWith("/")) addChat(text, me);
                        else {
                            perm.inputCmd(me, text);
                        };
                    };
                case "Tab":
                    initChatIn();
