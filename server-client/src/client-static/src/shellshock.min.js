        ...
        switch (cmd) {
            ...
            case Comm.Code.sync:
                ...
                if (id == meId) {
                    ...
                    for (var i = player.stateIdx = 0; i < FramesBetweenSyncs; i++) {
                        player.stateBuffer[i].controlKeys = input.unPackInt8U();
                        player.stateBuffer[i].yaw = input.unPackRadU();
                        player.stateBuffer[i].pitch = input.unPackRad();
                    };
                    player.x = x;
                    if (!player.jumping) {
                        player.y = y;
...
// [LS] Sound
function loadSounds(scene, onComplete) { //name from deobf leak
    var options = {
        spatialSound: true,
...
function loadMapMeshes(scene, onComplete) { //name from deobf leak
    mapMeshes = [null];
    var mat;
...
function buildMapData (errorFunc) { //name from deobf leak
    map = {
        width: minMap.width,
    ...
    Object.keys(minMap.data).forEach(function (meshName) {
        ...
        if (meshIdx) {
            ...
            if ("undefined" == typeof window && "SPECIAL" == mesh.theme && mesh.name.startsWith("spawn")) return void Object.values(meshData).forEach(function (cell) {
                spawnPoints[0].push({
                    x: cell.x,
                    y: cell.y,
                    z: cell.z
...
function createMapCells(callback) {
...
function loadMap(onComplete) { //name from deobf leak
    function complete() {
        hideSpinner();
        onComplete();
    };
...
function loadResources(scene, onComplete) { //2nd
...
function onLoadingComplete() { //3rd (callback of loadResources)
    console.log("onLoadingComplete");
    Collider = new ColliderConstructor(gameScene);
    loadMap(onMapComplete);
};

function onMapComplete() { //4th (callback of loadMap at end of onLoadingComplete)
...
function serverSync() {
    if (me) {
        ...
        output.packInt8(Comm.Code.sync);
        output.packInt8(me.stateIdx);
        output.packInt8(me.shotsQueued);
        ...
        for (startIdx, i = 0; i < FramesBetweenSyncs; i++) {
            ...
            output.packInt8(me.stateBuffer[idx].controlKeys);
...
function switchTeam() {
    closeAlertDialog();
    var output = new Comm.Out(1);
    output.packInt8(Comm.Code.switchTeam);
...
function bootPlayer(playerId, uniqueId) {
    if (isGameOwner && players[playerId] && players[playerId].uniqueId == uniqueId) {
        ...
        output.packInt8(Comm.Code.bootPlayer), output.packInt8(playerId), output.packInt16(players[playerId].uniqueId);
...
// [LS] VVVV the rest of this ISNT categorised
