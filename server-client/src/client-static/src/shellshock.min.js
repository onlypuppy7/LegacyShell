...
//receiving msgs
const LegacyShellOnMessage = function (e) {
...
Player.prototype.throwGrenade = function () {
    if (0 < this.shield && this.disableShield(), this.actor) (output = new Comm.output(3)).packInt8(CommCode.throwGrenade), output.packFloat(Math.clamp(grenadeThrowPower, 0, 1)), ws.send(output.buffer), me.grenadeCountdown = 80, this.actor.reachForGrenade();
        ...
        vec.x *= speed, vec.y *= speed, vec.z *= speed, pos.x = Math.floor(300 * pos.x) / 300, pos.y = Math.floor(300 * pos.y) / 300, pos.z = Math.floor(300 * pos.z) / 300, vec.x = Math.floor(300 * vec.x) / 300, vec.y = Math.floor(300 * vec.y) / 300, vec.z = Math.floor(300 * vec.z) / 300, (output = new Comm.output(14, true)).packInt8(CommCode.throwGrenade), output.packInt8(this.id), output.packFloat(pos.x), output.packFloat(pos.y), output.packFloat(pos.z), output.packFloat(vec.x), output.packFloat(vec.y), output.packFloat(vec.z), sendToAll(output.buffer), munitionsManager.throwGrenade(this, pos, vec)
    }
};
...
window.onload = function () {
    ...
    if (! function () {
        ...
        if (0 < missingFeatures) return str = 1 == missingFeatures ? "Your browser is missing a feature that Shell Shockers requires:<br><ul>" + str + "</ul>" : "Your browser is missing features that Shell Shockers requires:<br><ul>" + str + "</ul>", openAlertDialog("Oh, no!", '<div style="text-align: left">' + (str += "Downloading the latest version of your browser of choice will usually correct this. Internet Explorer is not supported.") + "</div>"), true;
        return false
    }()) {
        if (chatInEl = document.getElementById("chatIn"), chatOutEl = document.getElementById("chatOut"), killEl = document.getElementById("killTicker"), "undefined" != typeof noAdBlock && (adBlock = false), console.log("Blocking ads: " + adBlock), ga("send", "event", {
        ...
        }(), fetchTwitchStreams(), fetchHouseAds(), resize(), console.log("From EU: " + isFromEU), itemRenderer = new ItemRenderer, canvas = document.getElementById("canvas"), engine = new BABYLON.Engine(canvas, true, null, false), engineCaps = engine.getCaps(), BABYLON.Engine.audioEngine.setGlobalVolume(settings.volume), 0 == settings.volume ? BABYLON.Engine.audioEngine.audioContext.suspend() : BABYLON.Engine.audioEngine.audioContext.resume(), settings.autoDetail || settings.highRes || lowerResolution(), parsedUrl.query.openSettings && openSettingsMenu(), parsedUrl.query.test && (testing = true), parsedUrl.query.noSkybox && (noSkybox = true), parsedUrl.query.testMap) return playOffline = true, mapTest.x = parsedUrl.query.x, mapTest.y = parsedUrl.query.y, mapTest.z = parsedUrl.query.z, mapTest.pitch = parsedUrl.query.pitch, mapTest.yaw = parsedUrl.query.yaw, playerLimit = 1, 
        ws = {
            send: function () { }
        }, inGame = true, engine.stopRenderLoop(), void startGame();
        ...
        nameBox.value = getStoredString("lastUsername", ""), nameBox.addEventListener("keyup", function (event) {
            nameBox.value = fixStringWidth(event.target.value), "Enter" != event.code && 13 != event.keyCode || 0 < nameBox.value.length && (nameBox.disabled = true, play())
        }), unbloated1()
    };
};
...
function joinGame() {
    ...
    var joinGameId = document.getElementById("joinCode").value.trim(),
        mapId = 0;
    if (autoInvite = false, "none" != document.getElementById("createGame").style.display) joinType = CommCode.createPrivateGame, url = protocol + servers[server].address, (mapId = document.getElementById("mapSelect").selectedIndex - 1) < 0 ? mapId = Math.randomInt(0, minMaps.length) : ga("send", "event", {
        eventCategory: "Map",
        eventAction: "Select",
...
function closeAlertDialog() {
    document.getElementById("alert").style.display = "none", document.getElementById("alertFooter").style.display = "none", document.getElementById("username").disabled = false, hideDarkOverlay(), clearInterval(alertBarInterval)
};
...
function getXsollaToken(isForStore, productSku) {
    ...
    //             s.type = "text/javascript", s.async = true, s.src = "//static.xsolla.com/embed/paystation/1.0.7/widget.min.js", s.addEventListener("load", function (e) {
    ...
    //             }, false), document.getElementsByTagName("head")[0].appendChild(s), isForStore && (globalXsollaToken = xsollaToken, purchasingEnabled = true)
...
function disableProductPurchasing() {
    purchasingEnabled = false, openAlertDialog("Uh Oh!", "Something broke. Try reloading the game.", null, {
        label: "OK",
        width: "4em",
        onclick: closeAlertDialog
    })
...
function waitForSetupAndAuthComplete() {
    200 <= waitIteration ? openAlertDialog("", '<div id="setupAndAuthTimedOut">Uh oh, there was a problem. Try reloading the page.</div>', {
        ...
        var showDate;
        onResourcesLoaded(), isRunning = true, tryGiveReward() || function () {
...
function tryGiveReward() {
    if (true === checkedForReward) return false;
    if (null == reward || void 0 === reward.tag || null === reward.tag) return false;
    if (void 0 !== reward.openUrl && null !== reward.openUrl && true !== reward.wasUrlOpened) return openAlertDialog("Your Reward", '<div style="text-align: left">' + reward.openUrlMsg + "</div>", {
        label: "OK!!",
        width: "6em",
    ...
    servicesWs.onopen = function (e) {
        ...
        if (checkedForReward = true, rewardResult = JSON.parse(e.data), rewardResult.error) console.log("giveReward Error: " + JSON.stringify(rewardResult.error));
        else {
        ...
        return reward = null, unbloated1(), servicesWs.close(), true
    }, servicesWs.onclose = function (e) {
...
function openUrlAndGiveReward() {
    closeAlertDialog(), window.open(reward.openUrl, "_blank", "width=800,height=600"), reward.wasUrlOpened = true, tryGiveReward()
};
...
function unbloated1() {
    ...
    //         if (void 0 !== notification.data.nugget) return showNugget = true, void tryShowNugget();
    //         void 0 !== notification.data.rewardTag && (reward = new PlayerReward(notification.data.rewardTag, void 0 === notification.data.rewardOpenUrl ? null : notification.data.rewardOpenUrl, void 0 === notification.data.rewardOpenUrlMsg ? null : notification.data.rewardOpenUrlMsg), true === isRunning && tryGiveReward())
    //     }
    // }])
};
