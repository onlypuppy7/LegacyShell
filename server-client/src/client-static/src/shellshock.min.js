...
function setRespawnTime(sec) {
    ...
    respawnInterval = interval.set(function () {
        respawnTime--;
        (label.innerHTML = respawnTime) <= 0 && inGame && (respawnTime = -1, interval.clear(respawnInterval), button.className = "ssbutton green", label.innerHTML = "▶ PLAY!&nbsp;", doMapOverviewCamera())
    }, 1e3)
};
...
function wsSend (output, CommCode, passedWs) {
    ...
    if (passedWs && passedWs.send && Comm.Code[CommCode] !== Comm.Code.sync) {
