//some comments are added from the deobf leak (not many mind you)
...
function Bullet(scene) {
    this.scene = scene, this.x = 0, this.y = 0, this.z = 0, this.dx = 0, this.dy = 0, this.dz = 0, this.active = !1, this.player = null, this.damage = 20, this.range = 0, this.velocity = 0, this.actor = new BulletActor(this), this.end = new BABYLON.Vector3
}
Bullet.origin = new BABYLON.Vector3, Bullet.direction = new BABYLON.Vector3, Bullet.fire = function (player, pos, dir, weaponClass) {
...
Bullet.prototype.fireThis = function (player, pos, dir, weaponClass) {
    this.player = player, this.x = pos.x, this.y = pos.y, this.z = pos.z, Bullet.direction.copyFrom(dir).normalize(), this.dx = Bullet.direction.x * weaponClass.velocity, this.dy = Bullet.direction.y * weaponClass.velocity, this.dz = Bullet.direction.z * weaponClass.velocity, this.weaponClass = weaponClass, this.damage = weaponClass.damage, this.active = !0, this.range = weaponClass.range, this.velocity = weaponClass.velocity;
...
var catalog = new function () {
    ...
    this.isSetup = !1, this.filterItems = function (itemList, filterFunc) {
        ...
        if (!1 === this.isSetup && this.setupCatalog(), void 0 === classIdx || classIdx >= CharClass.length) return null;
        ...
        !1 === this.isSetup && this.setupCatalog();
        ...
        return this.filterItems(Items, function (item) {
            if (void 0 !== item.item_data.tags && null !== item.item_data.tags && 0 < item.item_data.tags.length)
                for (var i = 0; i < item.item_data.tags.length; i++)
                    if (item.item_data.tags[i] === tag) return !0;
            return !1
        })
...
var Collider = {
    ...
    pointCollidesWithMap: function (point, ignoreSoft) {
        if (isNaN(point.x) || isNaN(point.y) || isNaN(point.z)) return !1;
    ...
    meshCollidesWithMap: function (mesh, pos) {
        var cellsChecked = {};
        if (isNaN(pos.x) || isNaN(pos.y) || isNaN(pos.z)) return !0;
    ...
    meshCollidesWithCell: function (mesh, pos, cx, cy, cz, ignoreSoft) {
        ...
        if (cell.idx) {
            ...
            if (ignoreSoft && mapMesh.softness) return !1;
    ...
    rayCollidesWithMap: function (origin, direction, callback) {
        if (isNaN(origin.x) || isNaN(origin.y) || isNaN(origin.z)) return !1;
        if (origin.x < 0 || origin.x >= map.width || origin.z < 0 || origin.z >= map.depth || origin.y < 0 || origin.y >= map.height) return !1;
    ...
    projectileCollidesWithCell: function (origin, direction, voxel) {
        ...
        if (!cell) return !1;
        if (cell.mesh.softness) return !1;
        ...
        var pickInfo = Collider.intersectsColliderMesh(colliderMesh, !1);
        if (colliderMesh = pickInfo.pickedMesh, pickInfo.hit) {
            if (colliderMesh != Collider.fullCollisionMesh && (.5 < Math.abs(pickInfo.pickedPoint.x) || .5 < Math.abs(pickInfo.pickedPoint.y) || .5 < Math.abs(pickInfo.pickedPoint.z))) {
                ...
                BABYLON.Vector3.DistanceSquared(Collider.ray.origin, pickInfo.pickedPoint) < to0 && (pickInfo = Collider.ray.intersectsMesh(Collider.fullCollisionMesh, !1))
            }
            if (pickInfo.hit) {
    ...
    grenadeCollidesWithCell: function (origin, direction, voxel, ignoreSoft) {
        ...
        if (colliderMesh = pickInfo.pickedMesh, pickInfo.hit) {
            ...
            if (!pickInfo.insideMesh) return !1;
            var normal = pickInfo.normal;
            if ((pickInfo = Collider.ray.intersectsMesh(Collider.fullCollisionMesh, !1)).hit) {
                var p;
    ...
    intersectsColliderMesh: function (colliderMesh, precise) {
        var closestDistance = 1e5,
            ...
            closestNormal = null,
            insideMesh = !1;
        closestPick.hit && (closestNormal = closestPick.getNormal(!0, !1), 0 < (dot = BABYLON.Vector3.Dot(Collider.ray.direction, closestNormal)) && (insideMesh = !0), closestDistance = BABYLON.Vector3.DistanceSquared(Collider.ray.origin, closestPick.pickedPoint));
        for (var children = colliderMesh.getChildMeshes(), i = 0; i < children.length; i++) {
            ...
            if (pickInfo.hit) {
                var dot, distance = BABYLON.Vector3.DistanceSquared(Collider.ray.origin, pickInfo.pickedPoint),
                    normal = pickInfo.getNormal(!0, !1);
                0 < (dot = BABYLON.Vector3.Dot(Collider.ray.direction, normal)) && (insideMesh = !0), distance < closestDistance && (closestDistance = distance, closestPick = pickInfo, closestNormal = normal, 0 < dot && (insideMesh = !0))
            }
        }
    ...
    rayCollidesWithPlayerHelper: function (ray, player) {
        ...
        if (0 < (c = BABYLON.Vector3.Dot(m, m) - .140625) && 0 < b) return !1;
        var discr = b * b - c;
        if (discr < 0) return !1;
...
function Grenade(scene) {
    this.scene = scene, this.x = 0, this.y = 0, this.z = 0, this.dx = 0, this.dy = 0, this.dz = 0, this.ttl = 0, this.active = !1, this.player = null, this.actor = new GrenadeActor(this)
};

function Gun(player, subClass) {
    this.player = player, this.scene = this.player.scene, this.subClass = subClass, this.highPrecision = !1, this.equipTime = 25, this.stowWeaponTime = 25
};

function Eggk47(player, meshName) {
    Gun.call(this, player, Eggk47), this.ammo = {
        rounds: 30,
        capacity: 30,
        reload: 30,
        store: 240,
...
function CSG1(player, meshName) {
    Gun.call(this, player, CSG1), this.ammo = {
        ...
        storeMax: 15,
        pickup: 5
    }, this.hasScope = !0, this.longReloadTime = 240, this.shortReloadTime = 150, this.highPrecision = !0, this.actor = new CSG1Actor(this, meshName)
};
...
function RPEGG(player, meshName) {
    Gun.call(this, player, RPEGG), this.ammo = {
        ...
        storeMax: 3,
        pickup: 1
    }, this.hasScope = !0, this.longReloadTime = 170, this.shortReloadTime = 170, this.actor = new RPEGGActor(this, meshName)
};
...
Grenade.prototype.throw = function (player, pos, vec) {
    this.player = player, this.x = pos.x, this.y = pos.y, this.z = pos.z, this.dx = vec.x, this.dy = vec.y, this.dz = vec.z, this.ttl = 150, this.damage = 130, this.radius = 3, this.active = !0, this.actor && this.actor.throw()
};
...
Gun.prototype.fire = function () {
    if (this.actor) this.actor.fire();
        ...
        var output = new Comm.output(15, !0);
...
(Eggk47.prototype = Object.create(Gun.prototype)).constructor = Gun, Eggk47.weaponName = "EggK-47", Eggk47.standardMeshName = "eggk47", Eggk47.rof = 3600 / 550, Eggk47.recoil = 7, Eggk47.automatic = !0, Eggk47.accuracy = 12, Eggk47.shotSpreadIncrement = 40, Eggk47.accuracySettleFactor = .9, Eggk47.damage = 48, Eggk47.totalDamage = 48, Eggk47.range = 20, Eggk47.velocity = .5, Eggk47.prototype.fireMunitions = function (pos, dir) {
    Bullet.fire(this.player, pos, dir, Eggk47)
};
(DozenGauge.prototype = Object.create(Gun.prototype)).constructor = Gun, DozenGauge.weaponName = "Dozen Gauge", DozenGauge.standardMeshName = "dozenGauge", DozenGauge.rof = 15, DozenGauge.recoil = 10, DozenGauge.automatic = !1, DozenGauge.accuracy = 30, DozenGauge.shotSpreadIncrement = 120, DozenGauge.accuracySettleFactor = .88, DozenGauge.damage = 12, DozenGauge.totalDamage = 240, DozenGauge.range = 7, DozenGauge.velocity = .45, DozenGauge.prototype.fireMunitions = function (pos, dir) {
...
(CSG1.prototype = Object.create(Gun.prototype)).constructor = Gun, CSG1.weaponName = "CSG-1", CSG1.standardMeshName = "csg1", CSG1.rof = 60, CSG1.recoil = 10, CSG1.automatic = !1, CSG1.accuracy = 0, CSG1.shotSpreadIncrement = 180, CSG1.accuracySettleFactor = .94, CSG1.damage = 230, CSG1.totalDamage = 230, CSG1.range = 50, CSG1.velocity = .6, CSG1.prototype.fireMunitions = function (pos, dir) {
    Bullet.fire(this.player, pos, dir, CSG1)
};
(Cluck9mm.prototype = Object.create(Gun.prototype)).constructor = Gun, Cluck9mm.weaponName = "Cluck 9mm", Cluck9mm.standardMeshName = "cluck9mm", Cluck9mm.rof = 6, Cluck9mm.recoil = 6, Cluck9mm.automatic = !1, Cluck9mm.accuracy = 26, Cluck9mm.shotSpreadIncrement = 100, Cluck9mm.accuracySettleFactor = .84, Cluck9mm.damage = 38, Cluck9mm.totalDamage = 38, Cluck9mm.range = 15, Cluck9mm.velocity = .45, Cluck9mm.prototype.fireMunitions = function (pos, dir) {
    Bullet.fire(this.player, pos, dir, Cluck9mm)
};
(RPEGG.prototype = Object.create(Gun.prototype)).constructor = Gun, RPEGG.weaponName = "RPEGG", RPEGG.standardMeshName = "rpegg", RPEGG.rof = 80, RPEGG.recoil = 60, RPEGG.automatic = !1, RPEGG.accuracy = 5, RPEGG.shotSpreadIncrement = 200, RPEGG.accuracySettleFactor = .97, RPEGG.damage = 120, RPEGG.radius = 3, RPEGG.totalDamage = 330, RPEGG.range = 45, RPEGG.velocity = .2, RPEGG.readySpread = 5, RPEGG.prototype.fireMunitions = function (pos, dir) {
    if (Rocket.fire(this.player, pos, dir, RPEGG), this.actor)
        for (var i = 0; i < 10; i++) {
            var dx = .04 * Math.random() - .02,
                dy = .04 * Math.random() - .02,
                dz = .04 * Math.random() - .02;
            addExplosionSprite(explosionSmokeManager, 10, pos.x, pos.y, pos.z, dx, dy, dz, .4, !1)
        }
};
Math.PI2 = 2 * Math.PI, Math.PI90 = Math.PI / 2, Math.mod = function (n, m) {
    var remain = n % m;
...
mathSeed = 100, Math.seededRandom = function (min, max) {
...
function MunitionsManager(scene) {
    ...
    }, 200), this.rocketPool = new Pool(function () {
        return new Rocket(scene)
    }, 20), this.grenadePool = new Pool(function () {
...
MunitionsManager.prototype.getMapIntersectionPoint = function (proj, res) {
    if (res && !mapMeshes[res.cell.idx].softness && res.cell && res.cell.idx) {
        ...
        var pickInfo = ray.intersectsMesh(mapMesh, !1);
        if (pickInfo.hit) {
...
function Player(data, scene) {
    this.id = data.id, this.uniqueId = data.uniqueId, this.name = data.name, this.charClass = data.charClass, this.team = data.team, this.primaryWeaponItem = data.primaryWeaponItem, this.secondaryWeaponItem = data.secondaryWeaponItem, this.shellColor = data.shellColor, this.hatItem = data.hatItem, this.stampItem = data.stampItem, this.x = data.x, this.y = data.y, this.z = data.z, this.dx = data.dx, this.dy = data.dy, this.dz = data.dz, this.controlKeys = data.controlKeys, this.yaw = data.yaw, this.pitch = data.pitch, this.score = data.score, this.kills = data.kills, this.deaths = data.deaths, this.streak = data.streak, this.score = data.score, this.totalKills = data.totalKills, this.totalDeaths = data.totalDeaths, this.bestGameStreak = data.bestGameStreak, this.bestOverallStreak = data.bestOverallStreak, this.shield = data.shield, this.hp = data.hp, this.playing = data.playing, this.weaponIdx = data.weaponIdx, this.upgradeProductId = data.upgradeProductId, this.isGameOwner = data.isGameOwner, this.scene = scene, this.corrected = {
        ...
        pitch: 0,
        yaw: 0
    }, this.corrections = 0, this.hackDetected = data.hackDetected, this.rofCountdown = 0, this.triggerPulled = !1, this.shotsQueued = 0, this.reloadsQueued = 0, this.roundsToReload = 0, this.recoilCountdown = 0, this.reloadCountdown = 0, this.swapWeaponCountdown = 0, this.weaponSwapsQueued = 0, this.equipWeaponIdx = this.weaponIdx, this.shotSpread = 0, this.randomSeed = data.randomSeed, this.grenadeCount = 1, this.grenadeCapacity = 3, this.grenadeCountdown = 0, this.grenadesQueued = 0, this.jumping = !1, this.lastTouchedGround = 1, this.climbing = !1, this.climbingCell = {
        x: 0,
        z: 0,
        ry: 0
    }, this.bobble = 0, this.stateIdx = 0, this.syncStateIdx = 0, this.respawnTargetTime = 0, this.pauseTargetTime = 0, this.ready = !1, this.chatLineCap = 3, this.respawnQueued = !1, this.actor = new PlayerActor(this), this.stateBuffer = [];
...
Player.prototype.update = function (delta, resim) {
    ...
    if (this.controlKeys & CONTROL.left && (dx -= Math.cos(this.yaw), dz += Math.sin(this.yaw)), this.controlKeys & CONTROL.right && (dx += Math.cos(this.yaw), dz -= Math.sin(this.yaw)), this.controlKeys & CONTROL.up && (this.climbing ? dy += 1 : (dx += Math.sin(this.yaw), dz += Math.cos(this.yaw))), this.controlKeys & CONTROL.down && (this.climbing ? dy -= 1 : (dx -= Math.sin(this.yaw), dz -= Math.cos(this.yaw))), this.climbing) {
        ...
        if (0 == this.climbingCell.ry || this.climbingCell.ry, cy >= map.height) this.climbing = !1;
        else {
            var cell = map.data[cx][cy][cz];
            cell.idx && "ladder" == mapMeshes[cell.idx].colliderType && cell.ry == this.climbingCell.ry || (this.y = Math.round(this.y), this.climbing = !1)
        }
        this.collidesWithMap() && (0 < ndy && .3 < this.y % 1 ? (this.y -= ndy, this.dy *= .5) : ndy < 0 && (this.y -= ndy, this.dy *= .5, this.climbing = !1))
    } else {
...
Player.prototype.moveY = function (ndy, delta) {
    var old_y = this.y;
    this.y += ndy, this.collidesWithMap() ? (ndy < 0 && (this.actor && this.id == meId && 0 == this.lastTouchedGround && (this.lastTouchedGround = Date.now()), this.setJumping(!1)), this.y = old_y, this.dy *= .5) : 0 == this.jumping && this.setJumping(!0)
};
Player.prototype.canJump = function () {
    if (this.actor && this.id != meId) return !0;
    var canJump = !this.jumping | this.climbing;
    return canJump || (this.y -= .2, this.collidesWithMap() && (canJump = !0), this.y += .2), canJump
};
Player.prototype.jump = function () {
    return this.climbing ? (this.dy = .03, this.climbing = !1, this.setJumping(!0), !0) : !!this.canJump() && (this.dy = .06, this.setJumping(!0), !(this.lastTouchedGround = 0))
};
...
Player.prototype.changeCharacter = function (classIdx, primaryWeaponItem, secondaryWeaponItem, shellColor, hatItem, stampItem) {
    ...
    if (classIdx !== this.charClass || primaryWeaponItem.id !== this.primaryWeaponItem.id || secondaryWeaponItem.id !== this.secondaryWeaponItem.id || shellColor !== this.shellColor || itemChanged(hatItem) || itemChanged(stampItem)) {
        ...
        else (output = new Comm.output(8, !0)).packInt8(CommCode.changeCharacter), output.packInt8(this.id), output.packInt8(classIdx), output.packInt8(catalog.get8BitItemId(primaryWeaponItem, classIdx)), output.packInt8(catalog.get8BitItemId(secondaryWeaponItem, classIdx)), output.packInt8(shellColor), output.packInt8(catalog.get8BitItemId(hatItem, classIdx)), output.packInt8(catalog.get8BitItemId(stampItem, classIdx)), sendToOthers(output.buffer, this.id);
        this.changeWeaponLoadout(primaryWeaponItem, secondaryWeaponItem)
    }
};
Player.prototype.swapWeapon = function (idx) {
    var output;
    (this.actor && this.id != meId || this.canSwapOrReload() && idx < 2) && (this.equipWeaponIdx = idx, this.releaseTrigger(), this.swapWeaponCountdown = this.weapon.stowWeaponTime + this.weapons[idx].equipTime, this.actor ? (this.id == meId && reticle.hide(), this.weapon.actor.stow(), this.id == meId && ((output = new Comm.output(2)).packInt8(CommCode.swapWeapon), output.packInt8(idx), ws.send(output.buffer))) : (this.swapWeaponCountdown *= .9, this.weaponSwapsQueued--, (output = new Comm.output(3, !0)).packInt8(CommCode.swapWeapon), output.packInt8(this.id), output.packInt8(idx), sendToOthers(output.buffer, this.id)))
};
Player.prototype.collectItem = function (kind, applyToWeaponIdx) {
    switch (kind) {
        case ItemManager.AMMO:
            return !!this.weapons[applyToWeaponIdx].collectAmmo() && (this.actor && (Sounds.ammo.play(), updateAmmoUi()), !0);
...
Player.prototype.pullTrigger = function () {
    1 == grenadePowerUp && me.grenadeCountdown <= 0 ? this.cancelGrenade() : this.isAtReady() && this.rofCountdown <= 0 && (0 < this.weapon.ammo.rounds ? this.isSteady() ? (this.triggerPulled = !0, this.fire()) : this.weapon.actor.denyFire() : 0 < this.weapon.ammo.store ? this.reload() : this.weapon.actor.dryFire())
};
...
Player.prototype.reload = function () {
    if (this.actor && this.id != meId) this.weapon.actor.reload();
        ...
        else (output = new Comm.output(2, !0)).packInt8(CommCode.reload), output.packInt8(this.id), sendToOthers(output.buffer, this.id), this.reloadsQueued--;
...
Player.prototype.cancelGrenade = function () {
    grenadePowerUp = !1, me.grenadeCountdown = 30, this.id == meId && (document.getElementById("grenadeThrowContainer").style.visibility = "hidden"), this.actor && (this.actor.gripBone._frozen = !1)
};
Player.prototype.throwGrenade = function () {
    if (0 < this.shield && this.disableShield(), this.actor) (output = new Comm.output(3)).packInt8(CommCode.throwGrenade), output.packFloat(Math.clamp(grenadeThrowPower, 0, 1)), ws.send(output.buffer), me.grenadeCountdown = 80, this.actor.reachForGrenade();
        ...
        vec.x *= speed, vec.y *= speed, vec.z *= speed, pos.x = Math.floor(300 * pos.x) / 300, pos.y = Math.floor(300 * pos.y) / 300, pos.z = Math.floor(300 * pos.z) / 300, vec.x = Math.floor(300 * vec.x) / 300, vec.y = Math.floor(300 * vec.y) / 300, vec.z = Math.floor(300 * vec.z) / 300, (output = new Comm.output(14, !0)).packInt8(CommCode.throwGrenade), output.packInt8(this.id), output.packFloat(pos.x), output.packFloat(pos.y), output.packFloat(pos.z), output.packFloat(vec.x), output.packFloat(vec.y), output.packFloat(vec.z), sendToAll(output.buffer), munitionsManager.throwGrenade(this, pos, vec)
    }
};
Player.prototype.removeFromPlay = function () {
    this.playing = !1, this.controlKeys = 0, this.shotSpread = 0, this.jumping = !1, this.climbing = !1, this.actor && (this.actor.removeFromPlay(), this.id == meId && (reticle.hide(), scope.hide(), camera.fov = 1.25, grenadePowerUp = !1, document.getElementById("grenadeThrowContainer").style.visibility = "hidden"))
};
...
Player.prototype.lookForLadder = function (collide) {
    if (collide && collide.cell && this.controlKeys & CONTROL.up && "ladder" == mapMeshes[collide.cell.idx].colliderType) {
        ...
        if (!(.75 < diff && diff < 2.391)) {
            ...
            this.climbingCell.x = collide.x, this.climbingCell.z = collide.z, this.climbingCell.ry = collide.cell.ry, this.climbing = !0, this.lastTouchedGround = Date.now() + 100, this.setJumping(!1)
        }
    }
};
...
Pool.prototype.expand = function (num) {
    for (var i = 0; i < num; i++) {
        ...
        obj.id = i + this.size, obj.active = !1, this.objects.push(obj)
    }
    this.size += num
};
Pool.prototype.retrieve = function (id) {
    if (null != id) {
        ...
        return this.numActive++, this.objects[id].active = !0, this.objects[id]
    }
    var i = this.idx;
    do {
        i = (i + 1) % this.size;
        var obj = this.objects[i];
        if (!obj.active) return this.idx = i, this.numActive++, obj.active = !0, obj
    } while (i != this.idx);
    return this.expand(this.originalSize), this.retrieve()
};
Pool.prototype.recycle = function (obj) {
    obj.active = !1, this.numActive--
};
Pool.prototype.forEachActive = function (fn) {
    for (var i = 0; i < this.size; i++) {
        var obj = this.objects[i];
        !0 === obj.active && fn(obj, i)
    }
};

if ("undefined" == typeof window) var fs = require("fs");
var bigAdTimeout, duplicateWarningIssued = !1;
...
function loadMaterials(scene) {
    ...
    }, (mat = new BABYLON.StandardMaterial("wireframe", scene)).wireframe = !0, (mat = new BABYLON.StandardMaterial("normalBackface", scene)).diffuseColor = new BABYLON.Color3(.5, .5, .5), mat.ambientColor = new BABYLON.Color3(.5, .5, .5), mat.specularColor = new BABYLON.Color3(0, 0, 0), mat.backFaceCulling = !1, mat.twoSidedLighting = !0, (mat = new BABYLON.StandardMaterial("muzzleFlash", scene)).emissiveColor = BABYLON.Color3.White(), (mat = new BABYLON.StandardMaterial("ui", scene)).disableLighting = !0, mat.emissiveColor = BABYLON.Color3.White(), mat.fogEnabled = !1
};
        ...
        BABYLON.SceneLoader.ImportMesh("", rootUrl, path, scene, function (meshes, partcileSystems, skeletons) {
            try {
                for (var m = 0; m < meshes.length; m++) {
                    var mesh = meshes[m];
                    "egg" == mesh.name && numEggs++, "hands" == mesh.name && numHands++, !duplicateWarningIssued && (1 < numEggs || 1 < numHands) && (duplicateWarningIssued = !0, alert("Duplicate egg and/or hand models detected.\n\nOpen the weapon models in Blender and make sure egg/hands layers are turned off, then re-export.")), mesh.setMaterial && mesh.setMaterial(mat), mesh.setEnabled(!1), mesh.isPickable = !1, onMeshLoaded && onMeshLoaded(mesh)
                }
...
function Rocket(scene) {
    this.scene = scene, this.x = 0, this.y = 0, this.z = 0, this.dx = 0, this.dy = 0, this.dz = 0, this.ttArmed = 10, this.active = !1, this.player = null, this.damage = 130, this.actor = new RocketActor(this), this.end = new BABYLON.Vector3
};
...
Rocket.prototype.fireThis = function (player, pos, dir, weaponClass) {
    ...
    this.dx = v.x * weaponClass.velocity, this.dy = v.y * weaponClass.velocity, this.dz = v.z * weaponClass.velocity, this.damage = weaponClass.damage, this.active = !0, this.range = weaponClass.range, this.velocity = weaponClass.velocity;
    var res = Collider.rayCollidesWithMap(pos, dir, Collider.projectileCollidesWithCell);
    res && (this.actor && this.end.copyFrom(res.pick.pickedPoint), this.range = BABYLON.Vector3.Distance(pos, res.pick.pickedPoint)), this.actor && this.actor.fire()
};
...
var bigAdIdx, smallAdIdx, aipPrerollPlayer, houseAds, adTest = !1, houseAdsLoaded = !1, applixirAdWatched = !1;
...
function fetchHouseAds() {
    getRequest(parsedUrl.root + "housePromo.json?" + Date.now(), function (err, res) {
        ...
        }, houseAdsLoaded = !0) : (houseAds = JSON.parse(res), houseAdsLoaded = !0, function () {
...
parsedUrl.query.adTest && (adTest = !0);
var lastBannerLoaded, aipBannerDisplayed = !1,
    aipMultisizeBannerDisplayed = !1;
...
function loadBannerAd() {
    if ("none" == document.getElementById("overlay").style.display)
        if (document.getElementById("placeholder_300x250").style.display = "none", document.getElementById("multisizeBannerAdPlaceholder").style.display = "none", inGame && betweenRounds) displayBannerAd("bannerAdContainer", "houseAd_300x250"), ga("send", "event", {
            ...
            if (!0 !== playerAccount.hideAds) displayBannerAd("gameAdContainer", tag = "shellshock-io_multisize"), loadMultisizeAd(tag)
        } else {
            // if (!0 === playerAccount.hideAds && (0 === houseAds.houseAdPercentChance || 0 === houseAds.small.length)) return void (document.getElementById("menuAdContainer").style.display = "none");
            // if (100 * Math.random() <= houseAds.houseAdPercentChance || !0 === playerAccount.hideAds) return void displayBannerAd("bannerAdContainer", "houseAd_300x250");
            // var tag;
...
function isTimeToPlayVideoAd() {
    if (!0 === playerAccount.hideAds) return !1;
...
function BulletActor(bullet) {
    this.scene = bullet.scene, this.bullet = bullet, this.delayFrames = 0, this.mesh = this.scene.getMeshByName("bullet").createInstance(""), this.mesh.setEnabled(!1)
}
...
BulletActor.prototype.update = function () {
    --this.delayFrames <= 0 && this.mesh.setEnabled(!0), this.mesh.position.x = this.bullet.x, this.mesh.position.y = this.bullet.y, this.mesh.position.z = this.bullet.z, this.bullet.player.id != meId && (this.mesh.scaling.z = Math.min(this.mesh.scaling.z + .03, 3))
};
...
var Comm = {
    output: function (size) {
        this.buffer = new Uint8Array(size), this.idx = 0, this.packInt8 = function (val) {
    ...
    input: function (buf) {
        this.buffer = new Uint8Array(buf), this.idx = 0, this.isMoreDataAvailable = function () {
        ...
        function (scene, onComplete) {
            ...
            ! function (scene, onComplete) {
                ...
                ! function (scene, onComplete) {
                    ...
                    (mat = new BABYLON.StandardMaterial).diffuseColor = BABYLON.Color3.Red(), mat.emissiveColor = BABYLON.Color3.Red(), mat.specularColor = BABYLON.Color3.Black(), mat.wireframe = !0, barrier.material = mat, barrier.setEnabled(!1), mapMeshes.push(barrier);
                    ...
                    (mat = new BABYLON.StandardMaterial).diffuseColor = new BABYLON.Color3(0, .5, 1), mat.specularColor = new BABYLON.Color3(.1, .2, .4), mat.specularPower = 8, egg.material = mat, egg.setEnabled(!1), mapMeshes.push(egg), egg = BABYLON.MeshBuilder.CreateLathe("SPECIAL.spawn-red.none", {
                        shape: shape,
                        tessellation: 12
                    }, scene), (mat = new BABYLON.StandardMaterial).diffuseColor = new BABYLON.Color3(1, .25, .25), mat.specularColor = new BABYLON.Color3(.4, .3, .3), mat.specularPower = 8, egg.material = mat, egg.setEnabled(!1), mapMeshes.push(egg), loadMeshes(scene, ["map"], function (mesh) {
...
function openGameMenu() {
    escPressed = !1, document.pointerLockElement ? document.exitPointerLock() : showGameMenu()
};
...
function showGameSummary() {
    document.getElementById("gameSummary").style.visibility = "visible", document.getElementById("playerList").style.display = "none"
};
...
startGame = function () {
    ...
    gameType != GameType.teams ? (document.getElementById("teamButtonContainer").style.display = "none", document.getElementById("goButton").style.height = "6.5em") : (document.getElementById("teamButtonContainer").style.display = "block", document.getElementById("goButton").style.height = "4em"), document.body.style.overflow = "hidden", loadResources(gameScene, onLoadingComplete)
};
...
function requestRespawn() {
    ...
    var output = new Comm.output(1);
    output.packInt8(CommCode.requestRespawn), ws.send(output.buffer), console.log("respawn() requests respawn from server"), document.getElementById("help").style.display = "none"
};
...
function createParticleManager(filename, number, firstFrame, lastFrame, scene) {
    for (var manager = new BABYLON.SpriteManager("", "img/" + filename + "?" + version, number, 128, scene), i = 0; i < number; i++) {
        new BABYLON.Sprite("", manager).isVisible = !1
    }
    return manager.nextIdx = 0, manager.getSprite = function () {
        var s = this.sprites[this.nextIdx];
        return s.cellIndex = Math.randomInt(firstFrame, lastFrame), s.isVisible = !0, s.easing = Ease.linear, s.startSize = -1, s.endSize = -1, s.gravity = 0, s.dx = 0, s.dy = 0, s.dz = 0, s.float = 0, s.anim = 0, s.slow = 0, s.angle = 0, s.rotate = 0, s.stopped = !1, s.animColors = null, s.collide = !1, s.color.copyFrom(color4White), this.nextIdx = ++this.nextIdx % this.sprites.length, s
    }, manager
}
...
function addExplosion(x, y, z, damage, radius) {
    for (var i = 0; i < Math.floor(damage / 4); i++) {
        ...
        s.float = .015 * Math.random() + .001, s.collide = !0, (s = addExplosionSprite(explosionFireManager, .075 * animLength, x, y, z, 1.5 * dx, 1.5 * dy, 1.5 * dz, size)).collide = !0
    }
    if (me) {
    ...
    for (var i = 0; i < manager.sprites.length; i++) {
        ...
        if (s.isVisible) {
            ...
            if (0 <= s.endSize && (s.size = s.startSize + anim * (s.endSize - s.startSize)), s.animColors && lerp(s.animColors, anim, s.color), !s.stopped) {
                ...
                var res = !1;
                s.collide && (tv1.set(s.position.x, s.position.y, s.position.z), res = Collider.pointCollidesWithMap(tv1, !0)), res ? (s.stopped = !0, s.position.x -= s.dx, s.position.y -= s.dy, s.position.z -= s.dz) : s.gravity && (s.dy -= s.gravity), s.slow && (s.dx *= s.slow, s.dy *= s.slow, s.dz *= s.slow)
            }
            s.anim += delta / s.animLength, 1 <= s.anim && (s.isVisible = !1)
        }
    }
};
            ...
            showSpinner("Building Map", "Hold on!"),
                function (callback) {
                    ...
                    (SPS = new BABYLON.SolidParticleSystem("SPS", gameScene, {
                        updatable: !0
                    })).computeParticleColor = !1, SPS.computeParticleTexture = !1, SPS.computeParticleRotation = !1, SPS.recomputeNormals = !1;
                        ...
                        "undefined" == typeof window && (spawnPoints = [
                            ...
                            if (meshIdx) {
                                ...
                                switch (mesh.colliderType) {
                                    case "full":
                                        colliderMesh = Collider.fullCollisionMesh, colliderChildren = colliderPrecise = !1;
                                        break;
                                    case "wedge":
                                        colliderMesh = Collider.wedgeCollisionMesh, colliderChildren = !(colliderPrecise = !0);
                                        break;
                                    case "iwedge":
                                        colliderMesh = Collider.iwedgeCollisionMesh, colliderChildren = !(colliderPrecise = !0);
                                        break;
                                    case "ladder":
                                        colliderMesh = mesh.colliderMesh, colliderChildren = colliderPrecise = !1;
                                        break;
                                    case "aabb":
                                        colliderMesh = mesh.colliderMesh, colliderChildren = !(colliderPrecise = !1);
                                        break;
                                    case "obb":
                                        colliderMesh = mesh.colliderMesh, colliderChildren = colliderPrecise = !0
                                }
                            ...
                            } else meshesNotFound[meshName] = !0
                        });
                        var str = "";
                            ...
                            SPS.addShape(mesh, meshArr.length, {
                                positionFunction: function (particle, idx, shapeIdx) {
                                    var rx = meshArr[i].rx || 0,
                                        ry = meshArr[i].ry || 0,
                                        rz = meshArr[i].rz || 0;
                                    particle.position.x = meshArr[i].x + .5, particle.position.y = meshArr[i].y + .5, particle.position.z = meshArr[i].z + .5, particle.rotation.x = rx * rotInc, particle.rotation.y = ry * rotInc, particle.rotation.z = rz * rotInc, particle.alive = !1, i++
                                }
                            })
                        }
                    ...
                    showSpinner("Building Map", "Turning on the lights..."), requestAnimationFrame(function (onComplete) {
                        (mapMesh = SPS.buildMesh()).receiveShadows = !0, console.log("Casting rays");
                        var w = map.width + 1,
                            h = map.height + 1,
                            d = map.depth + 1,
                                        ...
                                        if (cell && cell.idx) {
                                            ...
                                            if ("SPECIAL" != cellMesh.theme) {
                                                for (var ri in cellMesh.rotation.x = cell.rx, cellMesh.rotation.y = cell.ry, cellMesh.rotation.z = cell.rz, cellMesh.position.x = cx + .5, cellMesh.position.y = cy + .5, cellMesh.position.z = cz + .5, cellMesh.computeWorldMatrix(), rays) {
                                                    var ray = rays[ri];
                                                    if (!ray.hit) {
                                                        var pickInfo = ray.intersectsMesh(cellMesh, !0);
                                                        pickInfo.hit && (ray.hit = !0, hits++)
                                                    }
                                                }
...
function onMapComplete() {
    ...
    try {
        var output;
        if (scope = new Scope, hitIndicator = new HitIndicator, reticle = new Reticle, gameScene.getMeshByName("muzzleFlash").material = gameScene.getMaterialByName("muzzleFlash"), gameScene.getMeshByName("bullet").material = gameScene.getMaterialByName("bullet"), gameScene.getMeshByName("rocket").material = gameScene.getMaterialByName("standardInstanced"), gameScene.getMeshByName("grenade").material = gameScene.getMaterialByName("emissive"), gameScene.getMeshByName("ammo").material = gameScene.getMaterialByName("standardInstanced"), gameScene.getMeshByName("grenadeItem").material = gameScene.getMaterialByName("standardInstanced"), munitionsManager = new MunitionsManager(gameScene), itemManager = new ItemManager, (bulletHoleManager = createParticleManager("particles.png", 50, 0, 0, gameScene)).fogEnabled = !0, bulletHoleManager.addHole = function (f, x, y, z) {
            var s = this.getSprite();
            s.position.x = x, s.position.y = y, s.position.z = z, s.angle = 6.282 * Math.random(), s.cellIndex = f, s.startSize = .03
        }, (explosionSmokeManager = createParticleManager("particles.png", 300, 1, 1, gameScene)).fogEnabled = !0, explosionSmokeManager.noAlphaTest = !0, (explosionFireManager = createParticleManager("particles.png", 300, 1, 1, gameScene)).fogEnabled = !0, explosionFireManager.blendMode = BABYLON.Engine.ALPHA_ADD, (shellFragManager = createParticleManager("particles.png", 400, 3, 6, gameScene)).fogEnabled = !0, yolkManager = createParticleManager("particles.png", 100, 2, 2, gameScene), yolkManager.fogEnabled = !0, playOffline) addPlayer({
            id: meId = 0,
            uniqueId: 0,
            name: "Test",
            charClass: 0,
        ...
        }), players[meId].hp = 100, doFirstPersonCamera(), startRendering(), addCanvasListeners();
        else mapOverview = !0, ws.onmessage = function (e) {
            for (var input = new Comm.input(e.data); input.isMoreDataAvailable();) {
                ...
                switch (cmd) {
                    ...
                    case CommCode.clientReady:
                        console.log("CommCode.clientReady received"), roundLength = input.unPackInt16U(), roundEndTime = input.unPackInt32U(), roundRestartTime = input.unPackInt32U(), 0 < roundLength ? (timedGame = !0, 0 < roundEndTime ? (document.getElementById("gameClock").style.display = "block", roundEndTime += Date.now()) : 0 < roundRestartTime ? (document.getElementById("gameClock").style.display = "none", document.getElementById("goButton").className = "ssbutton grey", document.getElementById("goButtonLabel").innerText = "Wait...", document.getElementById("playerList").style.display = "none", setRespawnTime(Math.floor(roundRestartTime / 1e3))) : respawnTime = -1) : (timedGame = !1, respawnTime = -1), openGameMenu(), startRendering(), autoInvite && inviteFriends();
                        break;
                    ...
                    case CommCode.roundEnd:
                        console.log("END ROUND"), prepareGameSummary(), betweenRounds = !0, document.getElementById("help").style.display = "none", document.getElementById("tipContainer").style.display = "block", document.getElementById("goButton").className = "ssbutton grey", document.getElementById("goButtonLabel").innerText = "Wait...", Sounds.endRound.play(), timeout.set(showContinueDialog, 3e3), setRespawnTime(NextRoundTimeout);
                        break;
                    case CommCode.roundStart:
                        console.log("START ROUND"), betweenRounds = !1, document.getElementById("gameClock").style.display = "block", roundEndTime = Date.now() + 1e3 * roundLength;
                        break;
        ...
        }, (output = new Comm.output(1)).packInt8(CommCode.clientReady), ws.send(output.buffer), (output = new Comm.output(1)).packInt8(CommCode.ping), pingStartTime = Date.now(), ws.send(output.buffer), setTimeout(unbloated2, 3e4)
    } catch (e) {
        console.log(e)
    }
};
...
function doMapOverviewCamera() {
    camera.parent = null, camera.position = new BABYLON.Vector3(0, map.height + 1, 0), camera.lockedTarget = null, camera.setTarget(new BABYLON.Vector3(map.width / 2, map.height / 4, map.depth / 2)), mapOverview = !0
}

function doFirstPersonCamera(player) {
    camera.position = BABYLON.Vector3.Zero(), camera.rotation = BABYLON.Vector3.Zero(), camera.rotationQuaternion = BABYLON.Quaternion.Zero(), camera.parent = player ? player.actor.eye : me.actor.eye, camera.lockedTarget = null, mapOverview = !1
}
    ...
    gameScene.registerBeforeRender(function () {
        (function () {
            ...
            for (; lastTimeStamp < currentTimeStamp;) {
                if (lastTimeStamp += 1e3 / 60, freezeFrame) me.update(1), me.actor.update(1);
                else
                    for (var i = 0; i < players.length; i++) {
                        var player = players[i];
                        player && (player.chatLineCap = Math.min(player.chatLineCap + 1 / 120, 3), player.playing && (player.update(1), player.actor.handsToWeaponSkeleton(), player.actor && player.actor.update(1)))
                    }
                me && (me.stateBuffer[me.stateIdx] = {
                    delta: 1,
                    yaw: me.yaw,
                    fire: !1,
                    jumping: me.jumping,
                ...
                }, me.stateIdx % FramesBetweenSyncs == 0 && ws.readyState == ws.OPEN && 0 < me.hp && !freezeFrame && serverSync(), hitIndicator.update(1), reticle.update(1), me.weapon && 0 == me.weapon.ammo.rounds && (me.stateIdx % 20 == 0 ? document.getElementById("ammo").style.color = "#f00" : me.stateIdx % 20 == 10 && (document.getElementById("ammo").style.color = "#fff")), 1 == grenadePowerUp && 0 < (grenadeThrowPower = Math.min(grenadeThrowPower + .015, 1)) && (document.getElementById("grenadeThrowContainer").style.visibility = "visible", document.getElementById("grenadeThrow").style.height = 100 * grenadeThrowPower + "%")), freezeFrame || (munitionsManager.update(1), itemManager.update(1), updateParticles(explosionSmokeManager, 1), updateParticles(explosionFireManager, 1), updateParticles(shellFragManager, 1), updateParticles(yolkManager, 1)), mapOverview && (mapOverviewAxis += .002, camera.position.x = Math.sin(mapOverviewAxis) * map.height + map.width / 2, camera.position.z = Math.cos(mapOverviewAxis) * map.height + map.depth / 2, camera.setTarget(new BABYLON.Vector3(map.width / 2, map.height / 4, map.depth / 2)))
            }
        }());
        if (me) {
...
function rebuildPlayerList() {
    ...
    for (i = 0; i < playerIdxs.length; i++) {
        ...
        if (list[i].style.display = "block", list[i].children[0].innerText = player.name, player.id != meId) {
            ...
            }(player.id, player.uniqueId), !1), list[i].style.pointerEvents = "all", list[i].style.cursor = "pointer"
        }
...
function mutePlayer(playerId, uniqueId) {
    var player = players[playerId];
    player && player.uniqueId == uniqueId && (player.muted ? (player.muted = !1, player.name = player.oldName) : (player.muted = !0, player.oldName = player.name, player.name = randomName()), player.actor.setupNameSprite(), rebuildPlayerList()), closeAlertDialog()
};
    ...
    me.playing && document.pointerLockElement && function (control) {
        switch (control) {
            ...
            case "fire":
                ...
                break;
            case "grenade":
                document.pointerLockElement && me && !grenadePowerUp && me.canSwapOrReload() && 0 < me.grenadeCount && (grenadePowerUp = !0, grenadeThrowPower = -.15);
                break;
            case "scope":
    ...
    if (key != lastKey)
        if (lastKey = key, grenadePowerUp || 0 != me.controlKeys || "Enter" != key || !settings.enableChat) {
            ...
            if (" " == k && (k = "SPACE", e.preventDefault()), debug) {
                ...
                if ("\\" == k) {
                    freezeFrame = !0;
                    for (var i = 0; i < gameScene.particleSystems.length; i++) gameScene.particlesPaused = !0;
                    for (i = 0; i < playerLimit; i++) players[i] && players[i].actor && (players[i].actor.mesh.setVisible(!0), players[i].actor.showNameSprite(), players[i].actor.positionNameSprite());
                    ws.close()
                }
            }
            inputDown(k)
...
function HitIndicator() {
    ...
    vertexData.positions = this.positions, vertexData.indices = [0, 1, 8, 0, 2, 1, 0, 2, 1, 0, 3, 2, 0, 3, 2, 0, 4, 3, 0, 4, 3, 0, 5, 4, 0, 5, 4, 0, 6, 5, 0, 6, 5, 0, 7, 6, 0, 7, 6, 0, 8, 7, 0, 8, 7, 0, 1, 8], vertexData.colors = this.colors, vertexData.applyToMesh(this.mesh, !0), this.mesh.layerMask = 536870912, this.mesh.material = gameScene.getMaterialByName("ui"), this.resize()
};
...
Scope.prototype.show = function () {
    ...
    this.crosshairs.scaling.x = h / 2, this.crosshairs.scaling.y = h / 2, this.crosshairs.setEnabled(!0), document.getElementById("scopeBorder").style.display = "block", camera.viewport.width = h / engine.getRenderWidth(), camera.viewport.x = .5 - .5 * camera.viewport.width
};
...
HitIndicator.prototype.update = function (delta) {
    ...
    me && me.playing && (camera.position.x *= mult, camera.position.z *= mult), this.mesh.updateVerticesData(BABYLON.VertexBuffer.ColorKind, this.colors, !0)
};
...
function isMeshVisible(mesh, offset) {
    ...
    for (var oy = offset || mesh.getBoundingInfo().boundingBox.center.z, dx = mesh.position.x - camera.globalPosition.x, dy = mesh.position.y + oy - camera.globalPosition.y, dz = mesh.position.z - camera.globalPosition.z, distance = Math.length3(dx, dy, dz), dv = Math.normalize3({
        ...
        if (cell.idx) {
            ...
            if ("full" == m.colliderType && !m.softness && 2 == ++collisions) return !1
        }
    }
    return !0
}
...
function addCanvasListeners() {
    console.log("Event listeners added"), canvas.style.pointerEvents = "all", canvas.addEventListener("mousedown", onCanvasMouseDown, !1), canvas.addEventListener("mouseup", onCanvasMouseUp, !1), canvas.addEventListener("mousemove", onCanvasMouseMove), canvas.addEventListener("wheel", onCanvasMouseWheel)
};
...
function Customizer(callback) {
    this.isReady = !1, this.camX = 0, this.camY = .4, this.camRadius = 3.4, this.rotY = 0, this.rotX = 0, this.jump = 0, this.turnCountdown = 60, this.selectedItem = 0, this.buyingItem = null, this.currentPickedItem = null, this.currentRenderFunc = null, this.currentHighlightFunc = null, this.currentSelectByIndexFunc = null, this.currentSelectByItemFunc = null, this.setupScene(), this.setupItems(), this.setupWeaponStats();
    var outer = this;
    loadResources(this.scene, function () {
        outer.onResourcesLoaded(), outer.isReady = !0, callback && callback()
    })
}
...
Customizer.prototype.openToTaggedItems = function (tag) {
    ...
    this.openItemSelector("Items: " + tag, taggedItems, null, null, !1, !1)
};
...
Customizer.prototype.clearItemLocks = function () {
    for (var i = 0; i < this.pageSize; i++) this.setItemLocked(i, !1, 0), this.setItemRequiresPhysicalUnlock(i, !1)
};
...
Customizer.prototype.openPhysicalItemStoreDialog = function () {
    openAlertDialog("Go To Store", "Open the store in a new tab?", {
        label: "Yes",
        width: "4em",
        onclick: physicalUnlockOpenStore
    }, {
        label: "No",
        width: "4em",
        onclick: closeAlertDialog
...
Customizer.prototype.openWeaponSelectorToItem = function (slot, weaponItem) {
    ...
    this.openItemSelector(title, weaponList, weaponItem, !1, !0)
};
...
Customizer.prototype.openHatSelectorToItem = function (hatItem) {
    this.openItemSelector("Hats", catalog.hats, hatItem, !0, !0)
};
...
Customizer.prototype.openStampSelectorToItem = function (stampItem) {
    this.openItemSelector("Stamps", catalog.stamps, stampItem, !0, !0)
};
...
Customizer.prototype.openItemSelector = function (title, itemList, item, firstItemBlank, highlightSelected) {
    if (playerAccount.isLoggedIn) {
        ...
        var uiItems = this.filterItemsForUI(itemList),
            blankFirstItemShim = !0 === firstItemBlank ? 1 : 0;
...
Customizer.prototype.encloseRenderPageFunc = function (itemList, firstItemBlank, highlightSelected) {
    ...
    return function () {
        outer.clearItemButtons(), outer.iteratePage(function (i, b) {
            if (!0 === firstItemBlank && 0 === i) outer.renderEmptyButton(), outer.setItemLocked(0, !1, 0), outer.setItemRequiresPhysicalUnlock(0, !1);
            else {
                var renderingItem = itemList[!0 === firstItemBlank ? i - 1 : i],
...
Customizer.prototype.encloseHighlightFunc = function (itemList, firstItemBlank, highlightSelected) {
    var outer = this;
    return function () {
        outer.clearHighlight(), !1 !== highlightSelected && outer.iteratePage(function (i, b) {
            if (!0 === firstItemBlank && 0 === i) null === outer.currentPickedItem && outer.highlightSelectedIndex(0);
            else {
                var item = itemList[!0 === firstItemBlank ? i - 1 : i];
...
Customizer.prototype.encloseSelectByIndexFunc = function (itemList, firstItemBlank, highlightSelected) {
    ...
    return function (displayIndex) {
        ...
        !0 === firstItemBlank && listIndex--;
        var noItem = !0 === firstItemBlank && 0 === displayIndex,
            item = noItem ? null : itemList[listIndex];
        !0 === noItem || outer.isItemPickable(item) ? (!0 === highlightSelected && outer.highlightSelectedIndex(displayIndex), !0 === noItem ? (outer.currentPickedItem.takeOffItem(), outer.currentPickedItem = null) : (item.pick(), item.poseEgg())) : (console.log("here, item: " + item.id), item.select(), item.poseEgg(), this.buyingItem = item, outer.itemSelected(item))
    }
};
Customizer.prototype.encloseSelectByItemFunc = function (itemList, firstItemBlank) {
    return function (item) {
        ...
        !0 === firstItemBlank && itemIdx++, this.currentSelectByIndexFunc(itemIdx)
    }
};
Customizer.prototype.itemSelected = function (item) {
    (menuOut("itemSelectorGrid"), menuOut("itemSelectorBackButton"), this.doesItemRequirePhysicalUnlock(item)) ? (menuFlexIn("physicalUnlock"), document.getElementById("physicalUnlockText").innerText = void 0 !== item.item_data.physicalUnlockText ? item.item_data.physicalUnlockText : "Buy a REAL THING to unlock this item!", document.getElementById("physicalUnlockButton").onclick = function () {
        return customizer.openPhysicalItemStoreDialog(), !1
        ...
        return extern.buyItem(item.id), !1
...
Customizer.prototype.filterItemsForUI = function (itemList) {
    for (var uiItems = [], i = 0; i < itemList.length; i++) !0 === itemList[i].showInUI && uiItems.push(itemList[i]);
    return uiItems
};
...
Customizer.prototype.extendItems = function (itemList, renderItemToCanvasFunc, poseEggWithItemFunc, selectItemFunc, takeOffItemFunc, pickItemFunc) {
    for (var i = 0; i < itemList.length; i++) {
        var showInUI = !0 === itemList[i].is_available || !0 === playerAccount.isItemOwned(itemList[i]);
...
Customizer.prototype.setupScene = function () {
    ...
    ground.rotation.x = Math.PI / 2, ground.receiveShadows = !0, ground.material = groundMat, this.scene.render();
...
Customizer.prototype.setExtraColorState = function (available) {
    for (var pickColorClosure = function (colorIdx) {
        ...
        !0 === available ? (colorSwatch.onclick = pickColorClosure(i), colorSwatch.classList.remove("lockedColor")) : (colorSwatch.onclick = extern.openBuyPassDialog, colorSwatch.classList.add("lockedColor"))
    }
};
...
function GrenadeActor(grenade, scene) {
    this.grenade = grenade, this.scene = grenade.scene, this.mesh = this.scene.cloneMesh("grenade"), this.mesh.setEnabled(!1), this.explodeSound = Sounds.grenade.explode.clone(), this.pinSound = this.mesh.attachSound(Sounds.grenade.pin), this.beepSound = this.mesh.attachSound(Sounds.grenade.beep), this.beep = !1, this.flashColor = null
}
...
function AmmoActor() {
    ItemActor.call(this, ItemManager.AMMO), this.mesh = this.scene.getMeshByName("ammo").createInstance(""), this.mesh.setEnabled(!1), shadowGen && shadowGen.getShadowMap().renderList.push(this.mesh)
}

function GrenadeItemActor() {
    ItemActor.call(this, ItemManager.GRENADE), this.mesh = this.scene.getMeshByName("grenadeItem").createInstance(""), this.mesh.setEnabled(!1), shadowGen && shadowGen.getShadowMap().renderList.push(this.mesh)
}
...
GunActor.prototype.stow = function () {
    var that = this;
    this.scene.beginAnimation(this.playerActor.gunContainer, 0, 25, !1, 1, function () {
        that.gunMesh.setEnabled(!1), that.gun.equip()
    })
};
GunActor.prototype.equip = function () {
    this.gunMesh.setEnabled(!0), this.playerActor.setWeaponSkeleton(this.skeleton), this.scene.beginAnimation(this.playerActor.gunContainer, 30, 55, !1, 1)
};
...
ItemManager.prototype.spawnItem = function (id, kind, x, y, z) {
    ...
    item.mesh.setEnabled(!0), item.mesh.position.x = x, item.mesh.position.y = y, item.mesh.position.z = z, testing && item.mesh.freezeWorldMatrix()
};
...
var engine, camera, customizer, itemRenderer, stampTexture, canvas, shadowGen, engineCaps, ws, selectedServer, meId, isGameOwner, viewingPlayerId, myTeam, username, gameStartTime, pingTotal, pingSamples, fpsTotal, fpsSamples, gameId, gameKey, mapIdx, gameType, privateGame, /*firebaseUi,*/ user, timesPlayed, chatInEl, chatOutEl, killEl, statsEl, purchasingEnabled, lastVer, globalXsollaToken, joinType, inputToControlMap, optimizer, inGame = !(ItemRenderer.prototype.renderStampToCanvas = function (stampItem, destCanvas) {
...
var i, s, o, r, a, m, shellColors = ["#ffffff", "#c4e3e8", "#e2bc8b", "#d48e52", "#cb6d4b", "#8d3213", "#5e260f", "#e70a0a", "#aa24ce", "#f17ff9", "#FFD700", "#33a4ea", "#3e7753", "#66dd33"];
    ...
    for (var i = 0; i < servers.length; i++) servers[i].ping = 0,
        function (serverIdx) {
            ...
            pinger.onopen = function (e) {
                ...
                server.ping = pingTime, pinger.close(), autoSelect && !serverSelected && (serverSelected = !0, selectServer(serverIdx)), pinged(), console.log(server.name + ": " + pingTime)
...
i = window;
s = document;
// o = "script", r = "ga", i.GoogleAnalyticsObject = r, i.ga = i.ga || function () {
//     (i.ga.q = i.ga.q || []).push(arguments)
// }, i.ga.l = 1 * new Date, a = s.createElement(o), m = s.getElementsByTagName(o)[0], a.async = 1, a.src = "https://www.google-analytics.com/analytics.js", m.parentNode.insertBefore(a, m), ga("set", "anonymizeIp", !isFromEU), ga("create", "UA-105800112-1", "auto"), ga("send", "pageview");
window.addEventListener("contextmenu", function (e) {
    e.preventDefault()
}, !1), window.onresize = function () {
    resize()
}, window.onload = function () {
    if (selectedServer = getStoredNumber("selectedServer", 0), lastVer = getStoredString("lastVersionPlayed", version), localStorage.setItem("lastVersionPlayed", version), lastVer != version) return (req = window.indexedDB.deleteDatabase("babylonjs")).onerror = function (event) {
    ...
    if (! function () {
        ...
        if (0 < missingFeatures) return str = 1 == missingFeatures ? "Your browser is missing a feature that Shell Shockers requires:<br><ul>" + str + "</ul>" : "Your browser is missing features that Shell Shockers requires:<br><ul>" + str + "</ul>", openAlertDialog("Oh, no!", '<div style="text-align: left">' + (str += "Downloading the latest version of your browser of choice will usually correct this. Internet Explorer is not supported.") + "</div>"), !0;
        return !1
    }()) {
        if (chatInEl = document.getElementById("chatIn"), chatOutEl = document.getElementById("chatOut"), killEl = document.getElementById("killTicker"), "undefined" != typeof noAdBlock && (adBlock = !1), console.log("Blocking ads: " + adBlock), ga("send", "event", {
            eventCategory: "Ad Block",
            eventAction: adBlock ? "yes" : "no"
        }), function () {
            ...
            for (var containerEl = document.getElementById("leaderboard"), team = 0; team < 3; team++) {
                ...
                teamEl.children[0];
                for (var i = 0; i < 20; i++) {
                    var rowEl = document.getElementById("teamRow").cloneNode(!0);
        ...
        }(), fetchTwitchStreams(), fetchHouseAds(), resize(), console.log("From EU: " + isFromEU), itemRenderer = new ItemRenderer, canvas = document.getElementById("canvas"), engine = new BABYLON.Engine(canvas, !0, null, !1), engineCaps = engine.getCaps(), BABYLON.Engine.audioEngine.setGlobalVolume(settings.volume), 0 == settings.volume ? BABYLON.Engine.audioEngine.audioContext.suspend() : BABYLON.Engine.audioEngine.audioContext.resume(), settings.autoDetail || settings.highRes || lowerResolution(), parsedUrl.query.openSettings && openSettingsMenu(), parsedUrl.query.test && (testing = !0), parsedUrl.query.noSkybox && (noSkybox = !0), parsedUrl.query.testMap) return playOffline = !0, mapTest.x = parsedUrl.query.x, mapTest.y = parsedUrl.query.y, mapTest.z = parsedUrl.query.z, mapTest.pitch = parsedUrl.query.pitch, mapTest.yaw = parsedUrl.query.yaw, playerLimit = 1, 
        ws = {
            send: function () { }
        }, inGame = !0, engine.stopRenderLoop(), void startGame();
        ...
        // var success, fail;
        // firebase ? (success = loggedIn, fail = loginFail, firebase.initializeApp({
        ...
        //         console.log("calling loggedIn from firebase.auth().onAuthStateChanged callback"), success(user)
        //     } else fail()
        // })) : loginFail()
        
        if (parsedUrl.hash) openJoinBox(); document.getElementById("joinCode").value = parsedUrl.hash;
        //bye bye bloat
        // aiptag.cmd.player.push(function () {
        ...
        nameBox.value = getStoredString("lastUsername", ""), nameBox.addEventListener("keyup", function (event) {
            nameBox.value = fixStringWidth(event.target.value), "Enter" != event.code && 13 != event.keyCode || 0 < nameBox.value.length && (nameBox.disabled = !0, play())
        }), unbloated1()
    }
};
var controlEl, feedbackValidateTimeout, settingsKeyNames = ["up", "down", "left", "right", "jump", "fire", "scope", "reload", "weapon", "grenade"],
    settingsKeyCells = {};
...
function configKey(e) {
    ...
    e = e || window.event, el == controlEl ? (1 == e.button && window.event.preventDefault(), el.className = "box defined", null != e.wheelDelta ? 0 < e.wheelDeltaY ? el.innerText = "WHEEL UP" : e.wheelDeltaY < 0 && (el.innerText = "WHEEL DOWN") : el.innerText = "MOUSE " + e.button, setControl(el)) : (controlEl && (controlEl.className = "box defined", controlEl.innerText = controlEl.oldText), el.oldText = el.innerText, el.className = "box active", el.innerText = getLocText("ui_settings_bindinput"), (controlEl = el).focus(), window.onkeydown = function (e) {
        var key = (e = e || window.event).key;
        if ("Escape" != key && "Tab" != key && "Enter" != key) return " " == key && (key = "space", e.preventDefault()), el.className = "box defined", el.innerText = key.toLocaleUpperCase(), setControl(el), e.stopPropagation(), !1;
...
function loggedIn(accountDataResponse) {
    ...
    customizer = new Customizer, extern.customizer = customizer, authAttemptComplete = !0
    // if (!playerAccount.isLoggedIn && !loggingIn) {
    //     if (loggingIn = !0, user.getIdToken(!0).then(function (idToken) {
    ...
    //     console.log("authWs closed: " + e.code + " " + e.reason), loggingIn = !1
    // }, authWs.onerror = function (e) {
    //     statsEl.innerHTML = '<p style="text-align: left">WebSocket connection failure. Please check your connection at:<br><a href="http://websocketstest.com" target="_window">websocketstest.com</a><br>as well as your firewall and antivirus settings.</p>', console.log("authWS Error: " + JSON.stringify(e, ["message", "arguments", "type", "name"])), loggingIn = !1
    // }
    //     }).catch(function (error) {
    //         console.log(error), loggingIn = !1
...
function getRequest(url, callback) {
    ...
    }, req.open("GET", url, !0), req.send(null), req)
};
...
function play() {
    ...
    0 == (username = document.getElementById("username").value.trim()).length && (username = randomName()), document.body.scrollTop = document.documentElement.scrollTop = 0, timesPlayed = getStoredNumber("timesPlayed", 0), localStorage.setItem("timesPlayed", timesPlayed + 1);
...
function joinGame() {
    var url;
    console.log("joinGame()"), joinResult = null;
    ...
    var joinGameId = document.getElementById("joinCode").value.trim(),
        mapId = 0;
    if (autoInvite = !(protocol = "wss://"), "none" != document.getElementById("createGame").style.display) joinType = CommCode.createPrivateGame, url = protocol + servers[server].address, (mapId = document.getElementById("mapSelect").selectedIndex - 1) < 0 ? mapId = Math.randomInt(0, minMaps.length) : ga("send", "event", {
        eventCategory: "Map",
        eventAction: "Select",
        eventLabel: minMaps[mapId].name
    }), autoInvite = !0;
    else if ("" != joinGameId) {
        joinType = CommCode.joinPrivateGame, joinGameId.startsWith("#") && (joinGameId = joinGameId.substr(1)), server = Number.parseInt(joinGameId.substr(0, 1), 36), gameId = Number.parseInt(joinGameId.substr(1, 3), 36), gameKey = Number.parseInt(joinGameId.substr(4, 2), 36);
        try {
    ...
    ws.onopen = function (e) {
        ...
        var len = 16 + 2 * username.length;
        playerAccount.isLoggedIn && (console.log("id and session available"), len += 2 * playerAccount.id.length + 5);

        var output = new Comm.output(len);
        output.packInt8(CommCode.joinGame);
        output.packInt8(uuid);
        ...
        output.packString(username);
        if (playerAccount.isLoggedIn) {
            output.packInt32(playerAccount.session);
        ...
        if (!freezeFrame)
            if (e.code == CloseCode.gameNotFound) console.log("Game not found - id: " + gameId + ", key: " + gameKey), openAlertDialog("Game Not Found", "Sorry! This game ID is either<br>invalid, or no longer exists.", {
                ...
                el.value = "", el.disabled = !1, el.focus(), document.getElementById("playButton").disabled = !1
        ...
        switch (joinResult = input.unPackInt8U()) {
            case CommCode.gameJoined:
                console.log("CommCode.gameJoined received"), document.getElementById("mainMenu").style.display = "none", meId = input.unPackInt8U(), myTeam = input.unPackInt8U(), gameType = input.unPackInt8U(), gameId = input.unPackInt16U(), gameKey = input.unPackInt16U(), mapIdx = input.unPackInt8U(), playerLimit = input.unPackInt8U(), isGameOwner = 1 == input.unPackInt8U(), console.log("is game owner: " + isGameOwner), inGame = !0, startGame()
        }
    }
}
...
function disableShadows() {
    return gameScene.shadowsEnabled = !1, mapMesh && inGame && (mapMesh.material = gameScene.getMaterialByName("mapNoShadow")), !0
}

function enableShadows() {
    gameScene.shadowsEnabled = !0, mapMesh && inGame && (mapMesh.material = gameScene.getMaterialByName("map"))
}
...
function closeAlertDialog() {
    document.getElementById("alert").style.display = "none", document.getElementById("alertFooter").style.display = "none", document.getElementById("username").disabled = !1, hideDarkOverlay(), clearInterval(alertBarInterval)
}
...
function showMainMenu() {
    if (engine && engine.stopRenderLoop(), document.body.style.overflow = "visible", window.scrollY = 0, viewingPlayerId = meId = 0, inGame) {
        ...
        gameScene.dispose(), inGame = !1, customizer = new Customizer(function () {
...
function openSettingsMenu() {
    ga("send", "event", "open settings"), inGame && 1 == settings.enableChat && initChatIn(), hideBannerAd(), hideMultisizeBannerAd(), menuIn("settingsMenu")
}
...
var interval = {
    ...
    set: function (func, delay) {
        ...
        return interval.intervals[newInterval] = !0, newInterval
    },
    ...
    timeout = {
        ...
        set: function (func, delay) {
            ...
            return timeout.timeouts[newTimeout] = !0, newTimeout
        },
        clear: function (handle) {
...
function buyChickenNuggetProduct() {
    ...
    callApi({
        cmd: "buyProduct",
        token: servicesToken,
        // firebaseId: playerAccount.firebaseId,
...
function showBoughtNuggetPopup() {
    setApplixirPopup(!1), document.getElementById("gotNuggetPopup").classList.toggle("hideme");
    // var adElement = document.getElementById("shellshock-io_multisize");
    // document.getElementById("chickenNuggetAdContainer").appendChild(adElement), adElement.style.display = "block", loadMultisizeAd("shellshock-io_multisize")
}

function getXsollaToken(isForStore, productSku) {
    // if (console.log("getXsollaToken called"), isForStore && purchasingEnabled) console.log("getXsollaToken: purchasing already enabled, skipping");
    // else {
    //     try {
    //         var servicesWs = new WebSocket(servicesServer)
    //     } catch (e) {
    ...
    //         var req = {
    //             cmd: "xsollaToken",
    //             // uid: playerAccount.firebaseId,
    //             isForStore: isForStore,
    //             productSku: productSku
    //         };
    //         servicesWs.send(JSON.stringify(req)), console.log("servicesWs opened, and xsollaToken request sent")
    //     }, servicesWs.onmessage = function (e) {
    //         getTokenData = JSON.parse(e.data), getTokenData.error ? (console.log("getTokenData Error: " + JSON.stringify(getTokenData.error)), disableProductPurchasing()) : "SUCCESS" == getTokenData.result ? (console.log("got Xsolla token: " + getTokenData.token), function (xsollaToken, isForStore) {
    //             var options = {
    ...
    //             s.type = "text/javascript", s.async = !0, s.src = "//static.xsolla.com/embed/paystation/1.0.7/widget.min.js", s.addEventListener("load", function (e) {
    //                 XPayStationWidget.on("init", function (e) {
    //                     console.log("store initialized"), XPayStationWidget.open()
    //                 }), XPayStationWidget.on("close", function (e) {
    //                     console.log("store closed"), closeBuyPassDialog(), setTimeout(function () {
    //                         checkBalance(), checkUpgrade()
    //                     }, 3e3)
    //                 }), XPayStationWidget.init(options)
    //             }, !1), document.getElementsByTagName("head")[0].appendChild(s), isForStore && (globalXsollaToken = xsollaToken, purchasingEnabled = !0)
    //         }(getTokenData.token, isForStore)) : (console.log("failed to get Xsolla token"), disableProductPurchasing()), servicesWs.close()
    //     }, servicesWs.onclose = function (e) {
    //         console.log("servicesWs closed: " + e.code + " " + e.reason)
    //     }, servicesWs.onerror = function (e) {
...
function disableProductPurchasing() {
    purchasingEnabled = !1, openAlertDialog("Uh Oh!", "Something broke. Try reloading the game.", null, {
        label: "OK",
        width: "4em",
        onclick: closeAlertDialog
    })
...
function waitForSetupAndAuthComplete() {
    200 <= waitIteration ? openAlertDialog("", '<div id="setupAndAuthTimedOut">Uh oh, there was a problem. Try reloading the page.</div>', {
        ...
        width: "8em",
        onclick: closeAlertDialog
    }) : null != customizer && !0 === customizer.isReady && authAttemptComplete && houseAdsLoaded ? (customizer.setupItems(), customizer.dressUpEgg(), setupUpgradeUI(), pingServers(!0, function () {
        var showDate;
        onResourcesLoaded(), isRunning = !0, tryGiveReward() || function () {
            var searchParams = new URLSearchParams(document.location.search);
            if (!1 === searchParams.has("showItem")) return !1;
            var showItemId = parseInt(searchParams.get("showItem"));
            return !1 !== (Number.isInteger(showItemId) && null !== catalog.findItemById(showItemId) && !1 === linkItemShown) && (linkItemShown = openItemInCustomizer(showItemId))
        }() || tryShowTaggedItems() || tryShowNugget() || (showDate = getStoredNumber("showBigAd", 0), Date.now() > showDate && !swiftShaderCheck() && (function () {
            for (var adIdxs = [], i = 0; i < houseAds.big.length; i++) houseAds.big[i].active && adIdxs.push(i);
            0 < adIdxs.length && (bigAdIdx = adIdxs[Date.now() % adIdxs.length], document.getElementById("bigAd").style.display = "block", document.getElementById("bigAdImg").src = "img/promo/" + houseAds.big[bigAdIdx].id + houseAds.big[bigAdIdx].imageExt, document.getElementById("bigAdLink"), houseAds.big[bigAdIdx].link && (document.getElementById("bigAdLink").href = houseAds.big[bigAdIdx].link, document.getElementById("bigAdLink").target = "_window"), houseAds.big[bigAdIdx].linksToTaggedItems && (document.getElementById("bigAdLink").href = "javascript: extern.hideBigAd(); extern.openTaggedItemsInCustomizer('" + houseAds.big[bigAdIdx].linksToTaggedItems + "');"), houseAds.big[bigAdIdx].linksToItemId && (document.getElementById("bigAdLink").href = "javascript: extern.openItemInCustomizer(" + houseAds.big[bigAdIdx].linksToItemId + ");"), ga("send", "event", {
                eventCategory: "Big ad",
                eventAction: "show",
                eventLabel: houseAds.big[bigAdIdx].label
            }), bigAdTimeout = setTimeout(function () {
                hideBigAd()
            }, 15e3))
        }(), localStorage.setItem("showBigAd", Date.now() + 432e5), 1)) || true //(document.getElementById("menuAdContainer").appendChild(document.getElementById("bannerAdContainer")), hideDarkOverlay(), showSmallAd())
    })) : (waitIteration++, setTimeout(waitForSetupAndAuthComplete, 100))
}

function openItemInCustomizer(itemId) {
    if (!playerAccount.isLoggedIn) return promoteLogin(), !1;
    document.getElementById("bigAd").style.display = "none", openCustomizerFromMainMenu();
    var item = catalog.findItemById(itemId);
    return customizer.openToItem(item), !0
}

function tryShowTaggedItems() {
    if (null === showTaggedItems) return !1;
    var shown = openTaggedItemsInCustomizer(showTaggedItems);
    return !0 === shown && (showTaggedItems = null), shown
}

function tryShowNugget() {
    return null !== showNugget && !playerAccount.isUpgraded() && (playerAccount.isLoggedIn ? (document.getElementById("bigAd").style.display = "none", openBuyPassDialog(), void (showNugget = null)) : (promoteLogin(), !1))
}

function openTaggedItemsInCustomizer(tag) {
    return playerAccount.isLoggedIn ? (openCustomizerFromMainMenu(), customizer.openToTaggedItems(tag), !0) : (promoteLogin(), !1)
}
...
function physicalUnlockOpenStore() {
    closeAlertDialog(), window.open(customizer.buyingItem.item_data.physicalItemStoreURL, "_blank")
}
...
function tryGiveReward() {
    if (!0 === checkedForReward) return !1;
    if (null == reward || void 0 === reward.tag || null === reward.tag) return !1;
    if (void 0 !== reward.openUrl && null !== reward.openUrl && !0 !== reward.wasUrlOpened) return openAlertDialog("Your Reward", '<div style="text-align: left">' + reward.openUrlMsg + "</div>", {
        label: "OK!!",
        width: "6em",
        onclick: openUrlAndGiveReward
    }, {
        label: "No, thanks",
        width: "6em",
        onclick: closeAlertDialog
    }), !0;
    if (!playerAccount.isLoggedIn) return openAlertDialog("Your Reward", "Uh oh, you gotta log in to get your reward!", {
        label: "Login",
        width: "6em",
        onclick: openLoginDialog
    }, {
    ...
    console.log("tryGiveReward called. ID: " + "playerAccount.firebaseId" + ", reward.tag: " + reward.tag);
    try {
        var servicesWs = new WebSocket(servicesServer)
    } catch (e) {
        console.log(e)
    ...
    servicesWs.onopen = function (e) {
        servicesWs.send(JSON.stringify({
            cmd: "reward",
            // firebase_id: playerAccount.firebaseId,
            reward_tag: reward.tag
        }))
    }, servicesWs.onmessage = function (e) {
        if (checkedForReward = !0, rewardResult = JSON.parse(e.data), rewardResult.error) console.log("giveReward Error: " + JSON.stringify(rewardResult.error));
        else {
            console.log("giveReward response: " + rewardResult.result), showDarkOverlay();
            var giveRewardDialog = document.getElementById("giveRewardDialog"),
                giveRewardResultText = document.getElementById("giveRewardResultText"),
                giveRewardCompleteButton = document.getElementById("giveRewardCompleteButton");
            switch (giveRewardDialog.style.display = "inline-block", rewardResult.result) {
                case "SUCCESS":
                    var msg = "<h1>EGGCELLENT!!!</h1>";
                    void 0 !== rewardResult.eggs_given && null !== rewardResult.eggs_given && 0 < rewardResult.eggs_given && (msg += '<div style="margin-top: 1em; margin-bottom: 2em; text-align: center;"><img src="img/egg_icon.png" style="width: 2em;" /><h2>+ ' + rewardResult.eggs_given + " </h2></div>", checkBalance());
                    var items = [];
                    if (void 0 !== rewardResult.item_ids && null !== rewardResult.item_ids && 0 < rewardResult.item_ids.length) {
                        for (var i = 0; i < rewardResult.item_ids.length; i++) {
                            var rewardedItem = catalog.findItemById(rewardResult.item_ids[i]);
                            playerAccount.isItemOwned(rewardedItem) || (playerAccount.inventory.push(rewardedItem), console.log("rewarded item id: " + rewardedItem.id)), items.push(rewardedItem)
                        }
                        customizer.setupItems(), msg += createShowItemListHTML(items.length).innerHTML
                    }
                    giveRewardResultText.innerHTML = msg, giveRewardCompleteButton.style = "ssbutton green", renderItemsInShowItemList(items);
                    break;
                case "REWARD_PREV_GIVEN":
                    giveRewardResultText.innerText = "You have already received this reward!", giveRewardCompleteButton.style = "ssbutton red";
                    break;
                case "REWARD_EXPIRED":
                    giveRewardResultText.innerText = "This reward is no longer available", giveRewardCompleteButton.style = "ssbutton red";
                    break;
                case "REWARD_NOT_FOUND":
                    giveRewardResultText.innerText = "Reward not found", giveRewardCompleteButton.style = "ssbutton red";
                    break;
                case "ERROR":
                    console.log("error: " + rewardResult.error), giveRewardResultText.innerText = "Uh oh, there was problem", giveRewardCompleteButton.style = "ssbutton red";
                    break;
                default:
                    giveRewardResultText.innerText = "Uh oh, there was problem. Try logging in again.", giveRewardCompleteButton.style = "ssbutton red"
            }
        }
        return reward = null, unbloated1(), servicesWs.close(), !0
    }, servicesWs.onclose = function (e) {
        console.log("servicesWs closed: " + e.code + " " + e.reason), reward = null
    }, servicesWs.onerror = function (e) {
        statsEl.innerHTML = '<p style="text-align: left">WebSocket connection failure. Please check your connection at:<br><a href="http://websocketstest.com" target="_window">websocketstest.com</a><br>as well as your firewall and antivirus settings.</p>', console.log("servicesWs Error: " + JSON.stringify(e, ["message", "arguments", "type", "name"])), unbloated1(), reward = null
    }
}

function openUrlAndGiveReward() {
    closeAlertDialog(), window.open(reward.openUrl, "_blank", "width=800,height=600"), reward.wasUrlOpened = !0, tryGiveReward()
}

function unbloated1() {
    // OneSignal.push(["addListenerForNotificationOpened", function (notification) {
    //     if (console.log("Received NotificationOpened:"), console.log(notification), void 0 !== notification.data && null !== notification.data) {
    //         if (void 0 !== notification.data.nugget) return showNugget = !0, void tryShowNugget();
    //         void 0 !== notification.data.rewardTag && (reward = new PlayerReward(notification.data.rewardTag, void 0 === notification.data.rewardOpenUrl ? null : notification.data.rewardOpenUrl, void 0 === notification.data.rewardOpenUrlMsg ? null : notification.data.rewardOpenUrlMsg), !0 === isRunning && tryGiveReward())
    //     }
    // }])
}
...
function checkUpgrade() {
    ...
    servicesWs.onopen = function (e) {
        ...
        getUpgradeData.error ? console.log("getUpgrade Error: " + JSON.stringify(getUpgradeData.error)) : (playerAccount.setUpgrade(getUpgradeData), setupUpgradeUI(), playerAccount.isUpgraded() && !1 === beforeStatus && loadBannerAd()), servicesWs.close()
    }, servicesWs.onclose = function (e) {
        console.log("servicesWs closed: " + e.code + " " + e.reason)
    }, servicesWs.onerror = function (e) {
        statsEl.innerHTML = '<p style="text-align: left">WebSocket connection failure. Please check your connection at:<br><a href="http://websocketstest.com" target="_window">websocketstest.com</a><br>as well as your firewall and antivirus settings.</p>', console.log("servicesWs Error: " + JSON.stringify(e, ["message", "arguments", "type", "name"]))
...
function setupUpgradeUI() {
    ...
    playerAccount.isLoggedIn ? playerAccount.isUpgraded() ? (buyPassDialog.classList.add("hideme"), upgradeIndicator.style.display = "block", customizer.setExtraColorState(!0)) : (inGame ? upgradeIndicator.classList.remove("ingame") : buyPassDialog.classList.remove("hideme"), upgradeIndicator.style.display = "none", customizer.setExtraColorState(!1)) : (buyPassDialog.classList.add("hideme"), upgradeIndicator.style.display = "none", customizer.setExtraColorState(!1)), adBlock && document.getElementById("buyNugget").remove()
}

function createShowItemListHTML(numItems) {
    for (var itemContainer = document.getElementById("showItemContainer").cloneNode(!0), itemList = itemContainer.querySelector("#showItemList"), i = 0; i < numItems; i++) {
        var appendIndexToId = function (element) {
            element.id += "_" + i
        },
            itemTemplate = document.getElementById("showItemTemplate").cloneNode(!0);
...
playerAccount = new function () {
    this.loggedOut = function () {
        ...
        !1 === foundInList && (weaponItem = catalog.forClass[this.classIdx].forWeaponSlot[slot][0]), this.pickedWeapons[this.classIdx][slot] = weaponItem
        ...
        return void 0 !== this.upgradeProductId && null !== this.upgradeProductId && !1 === this.upgradeIsExpired
    }, this.isItemOwned = function (item) {
        for (var i = 0; i < this.inventory.length; i++)
            if (this.inventory[i] && this.inventory[i].id === item.id) return !0;
        return !1
        ...
        if (1 <= version) {
            if (void 0 !== accountData.loadout && null !== accountData.loadout) {
                ...
                hasValue(accountData.loadout.colorIdx) && (this.colorIdx = accountData.loadout.colorIdx, 7 <= this.colorIdx && !1 === this.isUpgraded() && (this.colorIdx = 0))
            }
        ...
        7 <= this.colorIdx && !1 === this.isUpgraded() && (this.colorIdx = 0);
...
function RocketActor(rocket) {
    this.rocket = rocket, this.scene = rocket.scene, this.armed = !1, this.delayFrames = 0, this.mesh = this.scene.getMeshByName("rocket").createInstance(""), this.mesh.setEnabled(!1), this.explodeSound = Sounds.rpegg.explode.clone(), this.poofSound = Sounds.rpegg.poof.clone(), this.flySound = this.mesh.attachSound(Sounds.rpegg.fly)
}
...
PlayerActor.prototype.setupNameSprite = function () {
    ...
    nameTexture.clearRect(tx, ty - 256, 512, 256), this.drawTextOnNameTexture(this.player.name, 0, 32, 60, "white", !0)
};
...
PlayerActor.prototype.update = function (delta) {
    ...
    if (this.player.id == meId) this.scope && this.player.isAtReady(!0) ? (camera.fov = camera.fov + (this.player.weapon.actor.scopeFov - camera.fov) / 3, this.gunContainer.rotation.y *= .667, this.gunContainer.rotation.x *= .667, this.gunContainer.position.x += (-.25 - this.gunContainer.position.x) / 3, this.gunContainer.position.y += (this.player.weapon.actor.scopeY - this.gunContainer.position.y) / 4, this.gunContainer.position.z += (-.05 - this.gunContainer.position.z) / 3, this.player.weapon.hasScope && !this.zoomed && camera.fov < this.player.weapon.actor.scopeFov + .05 && (scope.show(), this.gunContainer.setEnabled(!1), this.zoomed = !0)) : (camera.fov = camera.fov + (1.25 - camera.fov) / 3, this.gunContainer.rotation.y += (2 * bcos - .14 - this.gunContainer.rotation.y) / 3, this.gunContainer.rotation.x += (.75 * bsin - .035 - this.gunContainer.rotation.x) / 3, this.gunContainer.position.x *= .667, this.gunContainer.position.y *= .667, this.gunContainer.position.z *= .667, this.zoomed && camera.fov > this.player.weapon.actor.scopeFov + .05 && (scope.hide(), this.gunContainer.setEnabled(!0), this.zoomed = !1));
...
PlayerActor.prototype.positionNameSprite = function () {
    if (this.nameSprite) {
        ...
        mp.y += .5 * this.nameSprite.height + .65;
        var v = new BABYLON.Vector3(camera.globalPosition.x - mp.x, camera.globalPosition.y - mp.y, camera.globalPosition.z - mp.z).normalize();
        v.scaleInPlace(.4), v.addInPlace(mp), this.nameSprite.position = v
    }
};
PlayerActor.prototype.showNameSprite = function () {
    this.player.playing && !this.nameSprite && (this.nameSprite = new BABYLON.Sprite("", nameSprites), this.nameSprite.invertV = !0, this.nameSprite.width = .6, this.nameSprite.height = .3, this.nameSprite.cellIndex = this.player.id, this.nameSprite.color = teamColors.outline[this.player.team])
};
...
PlayerActor.prototype.removeFromPlay = function () {
    this.scope = !1, this.zoomed = !1, this.throwingGrenade = !1, this.gripBone._frozen = !1, this.mesh.setEnabled(!1), this.hideNameSprite()
};
PlayerActor.prototype.restoreToPlay = function () {
    this.mesh.setEnabled(!0), this.gunContainer.setEnabled(!0), this.player.id != meId && this.showNameSprite()
};
...
PlayerActor.prototype.throwGrenade = function () {
    var that = this;
    this.player.id == meId && (document.getElementById("grenadeThrowContainer").style.visibility = "hidden"), this.hands.skeleton.beginAnimation("grenade_throw", !1, 1, function () {
        that.hands.skeleton && (that.throwingGrenade = !1, that.gripBone._frozen = !1)
    })
};
...
RocketActor.prototype.fire = function () {
    this.armed = !1, this.rocket.player.id == meId ? this.delayFrames = Math.ceil(ping / 17) + 3 : this.delayFrames = 3, this.mesh.position.x = this.rocket.x, this.mesh.position.y = this.rocket.y, this.mesh.position.z = this.rocket.z, this.mesh.lookAt(new BABYLON.Vector3(this.rocket.x + this.rocket.dx, this.rocket.y + this.rocket.dy, this.rocket.z + this.rocket.dz)), this.mesh.rotation.x += .015, this.mesh.rotation.y -= .015
};
RocketActor.prototype.update = function () {
    if (--this.delayFrames <= 0 && this.mesh.setEnabled(!0), !this.armed && this.rocket.ttArmed <= 0 && (this.armed = !0, this.flySound.play()), this.delayFrames % 4 == 0) {
        var dx = .04 * Math.random() - .02,
            dy = .04 * Math.random() - .02,
            dz = .04 * Math.random() - .02;
    ...
    return {
        ...
        openServerList: function () {
            ...
            for (var serverItem = document.getElementById("serverItem"), list = document.createElement("form"), barColors = ["lime", "yellow", "orange", "red"], i = 0; i < servers.length; i++) {
                var server = serverItem.cloneNode(!0);
            ...
            document.getElementById("alertMessage").appendChild(list), list.children[selectedServer].children[0].children[0].checked = !0, list.addEventListener("change", function (e) {
        ...
        respawn: function () {
            if (playOffline) return me.playing = !0, void canvas.requestPointerLock();
        ...
        setCheckOption: function (el) {
            ...
            switch (el.id) {
                ...
                case "highRes":
                    ...
                    break;
                case "enableChat":
                    el.checked ? (document.getElementById("enableChat").checked = !0, settings.enableChat = !0, localStorage.setItem("enableChat", !0), chatInEl.value = "Press ENTER to chat", inGame && (chatOutEl.style.display = "block", chatInEl.style.display = "block")) : (settings.enableChat = !1, localStorage.setItem("enableChat", !1), inGame && (chatOutEl.style.display = "none", chatInEl.style.display = "none"))
            }
        ...
        openPrivacyOptions: function () {
            ...
            //     ofAge: !1,
            //     targetedAds: !1
            // }, ofAgeCheck.checked = !1), consent.ofAge || (targetedAdsCheck.disabled = !0, document.getElementById("targetedAdsText").style.opacity = .25)
        },
        ...
        clickedBigAd: function () {
            ga("send", "event", {
                ...
                eventAction: "click",
                eventLabel: houseAds.big[bigAdIdx].label
            }), houseAds.big[bigAdIdx].linksToNugget && (showNugget = !0, hideBigAd(), tryShowNugget())
        },
        doConsent: function () {
            dismissNotification(function () {
                ...
                // ofAge: !0,
                // targetedAds: !0
            //}, document.getElementById("login").style.display = "block", aiptag.consented = !0, localStorage.setItem("consent", JSON.stringify(consent))
        },
        dontConsent: function () {
            ...
            //     ofAge: !1,
            //     targetedAds: !1
            // }, aiptag.consented = !1, localStorage.setItem("consent", JSON.stringify(consent))
        },
        setOfAge: function (e) {
        ...
        buyEggs: function () {
            // purchasingEnabled ? XPayStationWidget.open() : getXsollaToken(!0, null)
        },
        Customizer,
        playerAccount,
        ...
        openUrlAndGiveReward: openUrlAndGiveReward,
        physicalUnlockOpenStore: physicalUnlockOpenStore,
        ...
        buyPass: function () {
            // var sku;
            // console.log("buyPass called..."), getXsollaToken(!(sku = "golden_chicken_pass"), sku)
        },
        ...
        enterSpectatorMode: function () {
            ...
            }(), player.id != meId && (viewingPlayer.actor.hands.setRenderingGroupId(0), viewingPlayer.weapon.actor.gunMesh.setRenderingGroupId(0), viewingPlayer.actor.hat && viewingPlayer.actor.hat.setEnabled(!0)), player.actor.hands.setRenderingGroupId(1), player.weapon.actor.gunMesh.setRenderingGroupId(1), player.actor.hat && player.actor.hat.setEnabled(!1), player.actor.hideNameSprite(), viewingPlayerId = (viewingPlayer = player).id, doFirstPersonCamera(player), document.getElementById("spectatingName").innerText = player.name);
            var player
        },
        ...
        applixirAdStatusHandler: function (status) {
            switch (console.log("Applixir status: " + status), status) {
                ...
                case "ads-unavailable":
                    ...
                    break;
                case "sys-closing":
                    !0 === applixirAdWatched ? buyChickenNuggetProduct() : setApplixirPopup(!1), applixirAdWatched = !1;
                    break;
                case "ad-blocker":
                    setApplixirPopup(!1), openAlertDialog("Ad Blocker Detected", "No ads - No nugget", null, {
                        label: "Fine",
                        width: "4em",
                        onclick: function () {
                            closeAlertDialog()
