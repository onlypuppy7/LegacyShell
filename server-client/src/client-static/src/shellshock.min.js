    ...
    while (input.isMoreDataAvailable()) {
        ...
        switch (cmd) {
            ...
            case Comm.Code.addPlayer:
                ...
                var uniqueId = input.unPackInt16U();
                var name = input.unPackString();
                var charClass = input.unPackInt8U();
            ...
            case Comm.Code.respawn:
                var id = input.unPackInt8U(),
                    ...
                    z = input.unPackFloat(),
                    player = players[id];
                player && (player.respawn(x, y, z), id == meId && console.log("Comm.Code.respawn received from server for this player"));
                break;
...
Gun.prototype.fire = function () {
    if (this.actor) this.actor.fire();
        ...
        output.packInt8(Comm.Code.fire), output.packInt8(this.player.id), output.packFloat(pos.x), output.packFloat(pos.y), output.packFloat(pos.z), output.packFloat(dir.x), output.packFloat(dir.y), output.packFloat(dir.z), output.packInt8(mathSeed), sendToAll(output.buffer), this.fireMunitions(pos, dir)
    }
};
...
startGame = function () {
    ...
    (shadowLight = new BABYLON.DirectionalLight("", new BABYLON.Vector3(0, -1, 0), gameScene)).lightmapMode = BABYLON.Light.LIGHTMAP_SHADOWSONLY;
    ...
    (shadowGen = new BABYLON.ShadowGenerator(1024, shadowLight)).forceBackFacesOnly = true;
...
function serverSync() {
    if (me) {
        ...
        output.packInt8(Comm.Code.sync), output.packInt8(me.stateIdx), output.packInt8(me.shotsQueued);
...
function switchTeam() {
    closeAlertDialog();
    var output = new Comm.Out(1);
    output.packInt8(Comm.Code.switchTeam), ws.send(output.buffer)
};
...
function bootPlayer(playerId, uniqueId) {
    if (isGameOwner && players[playerId] && players[playerId].uniqueId == uniqueId) {
        ...
        output.packInt8(Comm.Code.bootPlayer), output.packInt8(playerId), output.packInt16(players[playerId].uniqueId), ws.send(output.buffer)
    }
    closeAlertDialog()
};
        ...
        switch (control) {
            ...
            case "jump":
                if (0 < me.lastTouchedGround && Date.now() > me.lastTouchedGround + 100 && me.jump()) {
                    var output = new Comm.Out(1);
                    output.packInt8(Comm.Code.jump), ws.send(output.buffer)
                }
                break;
            case "fire":
    ...
    for (var i = 0; i < servers.length; i++) servers[i].ping = 0,
        function (serverIdx) {
            ...
            pinger.onopen = function (e) {
                ...
                var output = new Comm.Out(1);
                output.packInt8(Comm.Code.ping), pinger.send(output.buffer), startTime = Date.now()
...
function joinGame() {
    ...
    ws.onopen = function (e) {
        ...
        if (!freezeFrame)
            if (e.code == Comm.Close.gameNotFound) console.log("Game not found - id: " + gameId + ", key: " + gameKey), openAlertDialog("Game Not Found", "Sorry! This game ID is either<br>invalid, or no longer exists.", {
                label: "OK"
            }), gameKey = gameId = 0, document.getElementById("joinCode").value = "";
            else if (e.code == Comm.Close.gameFull) console.log("Game full - id: " + gameId + ", key: " + gameKey), openAlertDialog("Game Full", "Sorry, this game is currently full!<br>Wait a moment, or try another one!", {
                label: "OK"
            });
            ...
            } else if (e.code == Comm.Close.mainMenu) console.log("WebSocket closing - returning to Main Menu");
            else if (e.code == Comm.Close.masterServerBusy) console.log("Master Server busy"), openAlertDialog("Matchmaker Busy", "Sorry! The match-maker is currently busy serving other players.<br>Please try again in a moment!", {
                label: "OK"
            }), gameKey = gameId = 0, document.getElementById("joinCode").value = "";
            else if (e.code == Comm.Close.masterServerOffline) console.log("Master Server offline"), openAlertDialog("Matchmaker Offline", "Oops! The match-maker is currently inaccessible.<br>Drop us a line and let us know!", {
                label: "OK"
            }), gameKey = gameId = 0, document.getElementById("joinCode").value = "";
...
function randomName() {
    ...
    for (; 69 == (num = Math.randomInt(1, 99)););
