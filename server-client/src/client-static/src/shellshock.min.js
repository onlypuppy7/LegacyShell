...
const distributed_data = LEGACYSHELLDISTRIBUTEDDATA;
...
const events = LEGACYSHELLEVENTS

/*
sometimes babylons cache just... doesnt work
the db becomes completely inaccessible and cant be deleted without going into browser settings
instead i added this thing to change the name of the db until it finds something that works in such cases where is refuses to cooperate
is this optimal? nope
it bloats storage a little but its preferable compared to being unable to play and not knowing why
you cant expect an end user to know about this bs
*/
var babylonDB = getStoredString("babylonDB", "babylonjs");
(async () => {
    var babylonDBsuccess = false;
    var append = Number(babylonDB.split("_")[1] || "0");
    ...
    while (!babylonDBsuccess) {
        try {
            indexedDB.open(babylonDB, 1).onsuccess = function() {
                babylonDBsuccess = true;
            };
            await wait(1000);
            console.log("babylonDB", babylonDB, babylonDBsuccess);
            if (babylonDBsuccess) {
                localStorage.setItem("babylonDB", babylonDB);
                if (shouldReload) {
                    console.log("babylonDB success");
                    // clearData.IndexedDB();
                    // console.log("other dbs cleared, reloading");
                    console.log("reload because le db?");
                    if (window.IDBrequestblocked) window.location.reload(true);
                };
            } else {
                shouldReload = true;
                append++;
                babylonDB = `babylonjs_${append}`;
                console.warn("babylonDB failed, trying", babylonDB);
            };
        } catch (e) {
            console.error("Error opening babylonDB", e);
            return;
        };
    };
})();
...
//beforebefore... yeah its a "hack"
LEGACYSHELLPLUGINSBEFOREBEFORE
...
var clearData = {
    ...
    full: function() {
        ...
        clearData.SessionStorage();
        clearData.IndexedDB();
        clearData.CacheStorage();
        clearData.ServiceWorkers();
        clearData.Notifications();
    },
};
...
var intervalGame = new IntervalManagerConstructor();
var timeoutGame = new TimeoutManagerConstructor();
...
var uvLowerBound = [0, 0.9375];
var uvUpperBound = [.0625, 1];
...
// rainParticleSystem/rainSound/wetMaterial/snowParticleSystem globals moved to the
// legacyweather plugin's shared.js.

var bgmSound;

function startBGM() {
    if ((!betweenRounds) && (!bgmSound)) {
        let bgm = minMap?.extents?.bgm;
        if (bgm && bgm !== "" && bgm !== "undefined") bgmSound = playSoundIndependent2D(bgm, {loop: true});
    };
};
...
var loadObjectMeshesBabylons = ["egg", "gun_cluck9mm", "gun_eggk47", "gun_csg1", "gun_dozenGauge", "gun_rpegg", "munitions", "muzzleFlash", "items", "reticle"];
...
// doThunderStrike() and setParticleSystems() moved to the legacyweather plugin's shared.js -
// both remain plain global functions once spliced in, so every existing call site elsewhere in
// this file (the doThunderStrike case removed from the message switch above, and
// disableShadows()/enableShadows()'s own setParticleSystems() calls further down) is unaffected.
...
const LegacyShellOnMessage = async function (msg) {
    ...
    while (input.isMoreDataAvailable()) {
        ...
        var cmdName = Comm.Convert(cmd);
        if (cmd !== Comm.Code.sync && cmd !== Comm.Code.syncData && cmd !== Comm.Code.ping) devlog("received cmd: ", cmdName, msg);
        await plugins.emit("LegacyShellOnMessage", {cmd, cmdName, input});
        if (!plugins.cancel) switch (cmd) {
            case Comm.Code.updateRoomParams:
                ...
                // Weather/time rendering (particle systems, skybox/fog, rain sound+material,
                // thunderstorm) moved to the legacyweather plugin's own roomParamsUpdated
                // listener - everything else in this case (cheatsEnabled above, the
                // Object.assign below) isn't weather-specific and stays here.
                await plugins.emit('roomParamsUpdated', { roomParamsObj });
            ...
            case Comm.Code.heal:
                ...
                break;
            case Comm.Code.explosionEffect:
                var playerId = input.unPackInt8U();
                var player = players_by_id[playerId];

                if (player) {
                    addExplosion(player.x, player.y, player.z, 40, 3);
                    playSoundIndependent("grenade.explode", {pos: new BABYLON.Vector3(player.x, player.y, player.z)});
                };
                break;
            ...
            case Comm.Code.roundEnd: //round has ended
                ...
                console.log("END ROUND", roundEndTime, Date.now(), roundEndTime - Date.now());
                ...
                await plugins.emit("roundEndSFX", {roundLength, roundEndTime, betweenRounds});
                if (!plugins.cancel) playSoundIndependent2D("endRound");
                ...
                if (betweenRounds && gameOptions.timedGame.spawnDuringInterval) nextRound = 6;
            ...
            case Comm.Code.roundStart: //round has started
                ...
                if (me.playing) startBGM();
                break;
            case Comm.Code.roundUpdate:
                roundLength = input.unPackInt16U(); //length of the round (in seconds)
                roundEndTime = input.unPackInt32U(); //time to end the round (in ms) as delta
                roundRestartTime = input.unPackInt32U(); //time to wait before next round starts (in ms) as delta
                ...
                if (roundLength > 0) {
                    ...
                    if (roundEndTime > 0) { //if roundEndTime is 0, then the game is over and we are waiting for the next game to start
                        document.getElementById("gameClock").style.display = "block";
                        document.getElementById("bestStreak").style.display = "none";
                    } else if (roundRestartTime > 0) { //between rounds
                        betweenRounds = true;
                        ...
                        let nextRound = Math.floor(roundRestartTime / 1000);
                        if (gameOptions.timedGame.spawnDuringInterval) nextRound = 6;
                        setRespawnTime(nextRound);
                    } else { //
                        respawnTime = -1;
                    };
            ...
            case Comm.Code.clientReady:
                ...
                break;
            // Comm.Code.doThunderStrike handling moved to the legacyweather plugin's own
            // LegacyShellOnMessage listener (see the plugins.emit a few lines above this switch).
            case Comm.Code.addPlayer:
                ...
                // 17 < id && devlog("playerId out of bounds: " + id + ", " + name); //shush..
                var playerData = {
                    id: id,
                    uniqueId: uniqueId,
                    name: name,
                    ...
                    primaryWeaponItem: catalog.findItemBy8BitItemId(ItemType.Primary, classIdx, input.unPackInt16U()),
                    secondaryWeaponItem: catalog.findItemBy8BitItemId(ItemType.Secondary, classIdx, input.unPackInt16U()),
                    shellColor: input.unPackInt8U(),
                    hatItem: catalog.findItemBy8BitItemId(ItemType.Hat, classIdx, input.unPackInt16U()),
                    stampItem: catalog.findItemBy8BitItemId(ItemType.Stamp, classIdx, input.unPackInt16U()),
                    x: input.unPackFloat(),
                    y: input.unPackFloat(),
                    z: input.unPackFloat(),
                ...
                if (!players_by_id[playerData.id]) {
            ...
            case Comm.Code.die:
                ...
                var killedPlayer = players_by_id[killedId];
                var byPlayer = players_by_id[byId];

                if (killedPlayer) {
                    killedPlayerName = killedPlayer.name;
                    var pos = new BABYLON.Vector3(killedPlayer.x, killedPlayer.y + (0.32 * killedPlayer.modifiers.scale), killedPlayer.z);
                    ...
                    plugins.emit("playerDeathAnimation", {killedPlayer});
                    if (!plugins.cancel) {
            ...
            case Comm.Code.sync:
                ...
                if (id == meId) {
                    ...
                    while (i !== player.stateIdx) {
                        ...
                        await player.update(state.delta, true);
            ...
            case Comm.Code.reload:
                ...
                if (id == meId) {
                    ...
                    players_by_id[id] && players_by_id[id].reload();
                };
                break;
            case Comm.Code.swapWeapon:
                var id = input.unPackInt8U(),
                    weaponIdx = input.unPackInt8U();
                players_by_id[id] && players_by_id[id].swapWeapon(weaponIdx);
                break;
            ...
            case Comm.Code.respawn:
                ...
                if (player) {
                    ...
                    if (id == meId) {
                        console.log("Comm.Code.respawn received from server for this player");
                    };
                };
                break;
            case Comm.Code.changeCharacter:
                var id = input.unPackInt8U();
                var classIdx = input.unPackInt8U();
                var primaryWeaponIdx = input.unPackInt16U();
                var secondaryWeaponIdx = input.unPackInt16U();
                var shellColor = input.unPackInt8U();
                var hatIdx = input.unPackInt16U();
                var stampIdx = input.unPackInt16U();
            ...
            case Comm.Code.setModifiers:
                ...
                const extraModifiers = JSON.parse(input.unPackLongString());
                devlog("extraModifiers", extraModifiers);
...
function Customizer(callback) {
    ...
    this.currentPickedItem = null;
    this.currentRenderFunc = null;
    this.currentHighlightFunc = null;
    this.currentSelectByIndexFunc = null;
    this.currentSelectByItemFunc = null;
    this.setupScene();
    this.setupItems();
    this.setupWeaponStats();

    var outer = this;
    devlog("loading resources for customizer");
    loadResources(this.scene, function () {
        devlog("resources loaded for customizer");
        outer.onResourcesLoaded();
        outer.isReady = true;
        callback && callback();
    });
};
...
Customizer.prototype.open = function () {
    ...
    document.getElementById("redeemCodeButton").style.display = playerAccount.session ? "inline" : "none";
    this.camX = .39;
    this.camY = .3;
    this.camRadius = 3.4;
    this.initWeaponButton(Slot.Primary);
    this.initWeaponButton(Slot.Secondary);
    this.initHatButton();
...
Customizer.prototype.close = function () {
    ...
    document.getElementById("redeemCodeButton").style.display = "none";
    ...
    inGame ? (
        ...
        startRendering(),
        me.changeCharacter(playerAccount.classIdx,
            playerAccount.getPrimaryWeapon(),
            playerAccount.getSecondaryWeapon(),
            playerAccount.colorIdx,
            playerAccount.hatItem,
            playerAccount.stampItem, 0, 0
        ),
        chatting && chatInEl.focus(),
        showBannerAdWithPlaceholder("multisizeBannerAdPlaceholder"),
        loadBannerAd()
...
Customizer.prototype.updatePageArrows = function () {
    document.getElementById("itemPageLeft").style.visibility = 0 < this.page ? "visible" : "hidden";
...
Customizer.prototype.clearItemButtons = function () {
    for (var i = 0; i < this.pageSize; i++) {
        const canvas = document.getElementById("item" + i);
        canvas.getContext("2d").clearRect(0, 0, 256, 256);

        plugins.emit('clearItemButtons', { i, canvas });
    };
    this.clearItemLocks()
};
        ...
        outer.clearItemButtons(), outer.iteratePage(function (i, b) {
            if (true === firstItemBlank && 0 === i) outer.renderEmptyButton(), outer.setItemLocked(0, false, 0), outer.setItemRequiresPhysicalUnlock(0, false);
                ...
                var locked = !outer.isItemPickable(renderingItem),
                    requiresPhysicalUnlock = outer.doesItemRequirePhysicalUnlock(renderingItem); devlog("renderingItem", renderingItem, locked, requiresPhysicalUnlock);
                ...
                plugins.emit('encloseRenderPageFunc', { i, b, canvas, renderingItem, locked, requiresPhysicalUnlock });
            }
...
Customizer.skyColor = new BABYLON.Color3(.35, .65, .8); //background colour. pre 0.15.7: new BABYLON.Color3(.52, .68, .82) post: .35, .65, .8
Customizer.diffuseColor = new BABYLON.Color3(.35, .5, .6); //pre 0.15.7: new BABYLON.Color3(.4, .5, .6) post: .35, .5, .6
// let CustomizerScene;
Customizer.prototype.setupScene = function () {
    // this.scene = CustomizerScene || new BABYLON.Scene(engine);
    // CustomizerScene = this.scene;
    this.scene = new BABYLON.Scene(engine);
    this.scene.skipMapLoading = true;
    this.scene.fogMode = BABYLON.Scene.FOGMODE_EXP2, this.scene.fogDensity = .06;
    this.scene.sunLight = {
        direction: new BABYLON.Vector3(0.2, 1, 0.1),
        diffuse: new BABYLON.Color3.White()
    };
    this.scene.fogColor = Customizer.skyColor;
    this.scene.clearColor = Customizer.skyColor;
    ...
    groundMat.diffuseColor = Customizer.diffuseColor;
    ...
    stampTexture = new BABYLON.Texture("img/stamps.png?v=LEGACYSHELLSTAMPSPNG", this.scene);
...
Customizer.prototype.setupWeaponStats = function () {
    var setupWeaponStatsInList = function (weaponList) {
        for (var i = 0; i < weaponList.length; i++) {
            var weapon = item_classes[weaponList[i].item_data?.class || weaponList[i].category_name]; //console.log("i", i, weapon)
            for (var key in weaponStats) {
                //console.log(key)
                weaponStats[key].flip ? (weaponStats[key].max = Math.max(weaponStats[key].max, -weapon[key]), weaponStats[key].min = Math.min(weaponStats[key].min, -weapon[key])) : (weaponStats[key].max = Math.max(weaponStats[key].max, weapon[key]), weaponStats[key].min = 0)
            };
        }
    };
    setupWeaponStatsInList(catalog.primaryWeapons);
    setupWeaponStatsInList(catalog.secondaryWeapons);
};

// [LS] ItemRenderer - moved to src/shell/itemRenderer.js (see CLAUDE.md's "Shared game logic"
// splice mechanism) so legacyadmin's item browser/codes tab can reuse the exact same
// implementation instead of a second copy. Behavior-preserving here: loadMaterials is still a
// bundle-global function at this point in the concatenated script, so the extracted constructor's
// `typeof loadMaterials === 'function'` check still finds it and calls it exactly as before.
LEGACYSHELLITEMRENDERER
...
function tryShowTaggedItems(taggedItems) {
    if (taggedItems) showTaggedItems = taggedItems;
...
function PlayerActor(player) {
    ...
    plugins.emit('playerActorBodyMeshCreated', { playerActor: this, bodyMesh: this.bodyMesh, player });
    ...
    if (shadowGen) {
        ...
        plugins.emit('playerActorShadowRenderList', { playerActor: this, shadowGen: shadowGen });
    };
...
PlayerActor.prototype.applyStamp = function (stampItem) {
    ...
    var stampWidthHeight = stampTexture._texture.width / stampSize;

    uvLowerBound = [0, 1 - (1 / stampWidthHeight)];
    uvUpperBound = [1 / stampWidthHeight, 1];

    // console.log(stampWidthHeight);

    if (null !== stampItem) {
        this.bodyMesh.stampU = stampItem.item_data.x / stampWidthHeight;
        this.bodyMesh.stampV = 1 - stampItem.item_data.y / stampWidthHeight;
    };
};
...
PlayerActor.prototype.positionNameSprite = function () {
  if (this.nameSprite) {
    ...
    mp.y += 0.5 * this.nameSprite.height + (0.65 * this.player.modifiers.scale);
...
PlayerActor.prototype.removeFromPlay = function () {
    ...
    plugins.emit('playerActorRemoveFromPlay', { playerActor: this });
    rebuildPlayerList();
};
PlayerActor.prototype.restoreToPlay = function () {
    ...
    this.updateTeam();
    plugins.emit('playerActorRestoreToPlay', { playerActor: this });
    // if (playOffline) this.mesh.setEnabled(true);
    rebuildPlayerList();
};
...
PlayerActor.prototype.reachForGrenade = function () {
    ...
    this.hands.skeleton.beginAnimation("grenade.grab", false, this.player.modifiers.physicsSpeedModifier, function () {
        that.hands.skeleton && that.hands.skeleton.disableBlending()
    });
};
PlayerActor.prototype.throwGrenade = function () {
    var that = this;
    this.player.id == meId && (document.getElementById("grenadeThrowContainer").style.visibility = "hidden"), this.hands.skeleton.beginAnimation("grenade.throw", false, this.player.modifiers.physicsSpeedModifier, function () {
...
BulletActor.prototype.fire = function () {
    // if (this.rocket.player.id == meId) {
    //     this.delayFrames = Math.ceil(ping / 17) + 3 ;
    // } else {
    //     this.delayFrames = 3;
    // };

    // this.delayFrames = Math.min(2 / this.bullet.player.modifiers.bulletSpeedModifier, 4);
    this.delayFrames = 2;
...
BulletActor.prototype.update = function () {
    if (--this.delayFrames <= 0) this.mesh.setEnabled(true);
    this.mesh.position.x = this.bullet.x;
    this.mesh.position.y = this.bullet.y;
    this.mesh.position.z = this.bullet.z;
    if (this.bullet.player.id != meId) {
...
GunActor.prototype.setup = function (muzFlashZ) {
    ...
    plugins.emit('gunActorSetup', { gunActor: this, shadowGen });
...
GunActor.prototype.stow = function () {
    var that = this;
    this.scene.beginAnimation(this.playerActor.gunContainer, 0, 25, false, this.playerActor.player.modifiers.physicsSpeedModifier, function () {
        that.gunMesh.setEnabled(false), that.gun.equip()
    });
};
GunActor.prototype.equip = function () {
    this.gunMesh.setEnabled(true), this.playerActor.setWeaponSkeleton(this.skeleton), this.scene.beginAnimation(this.playerActor.gunContainer, 30, 55, false, this?.playerActor?.player?.modifiers?.physicsSpeedModifier || 1)
};
...
GunActor.prototype.fire = function () {
    ...
    this.skeleton.beginAnimation("fire", false, this.playerActor.player.modifiers.physicsSpeedModifier)
};
GunActor.prototype.reload = function () {
    if (this.gun.longReloadTime == this.gun.shortReloadTime) {
        this.skeleton.beginAnimation("reload", false, this.playerActor.player.modifiers.physicsSpeedModifier * this.playerActor.player.modifiers.reloadSpeedModifier);
    } else if (0 < this.gun.ammo.rounds) {
        this.skeleton.beginAnimation("shortReload", false, this.playerActor.player.modifiers.physicsSpeedModifier * this.playerActor.player.modifiers.reloadSpeedModifier);
    } else {
        this.skeleton.beginAnimation("longReload", false, this.playerActor.player.modifiers.physicsSpeedModifier * this.playerActor.player.modifiers.reloadSpeedModifier);
    }
};
...
Reticle.prototype.update = function (delta) {
    if (me.weapon) {
        for (var i = 0; i < 4; i++) {
            ...
            var size = spread * (1.25 / camera.fov);
            this.crosshairs[i].style.transform = "rotate(" + 90 * i + "deg) translateY(" + (.069 * size + .12) + "em)"
        }
...
function loadBannerAd() {
    if ("none" == document.getElementById("overlay").style.display) {
        ...
        if (inGame && betweenRounds) {
            devlog("no ad ur in a game or something");
            displayBannerAd("bannerAdContainer", "houseAd_300x250");
            // ga("send", "event", {
            ...
            if (!0 === playerAccount.hideAds && (0 === houseAds.houseAdPercentChance || 0 === houseAds.small.length)) {
                devlog("no ads?");
                document.getElementById("menuAdContainer").style.display = "none";
                return;
            };
            if (100 * Math.random() <= houseAds.houseAdPercentChance || !0 === playerAccount.hideAds) {
                devlog("show house ad pliz");
                displayBannerAd("bannerAdContainer", "houseAd_300x250");
                return;
            };
...
var latestZipTimes = String(LEGACYSHELLZIPTIMES);

function setGameTypeSelect() {
    var gameTypeSelect = document.getElementById("gameTypeSelect");

    var oldSelectedIndex = gameTypeSelect.selectedIndex;
    // devlog("oldSelectedIndex gameType", oldSelectedIndex, gameTypeSelect.selectedIndex, gameType);
    gameTypeSelect.innerHTML = "";

    var randomisedEl = document.createElement("option");
    randomisedEl.textContent = `[${serverInfo?.playerCountTotal?.public}] {Random Mode}`;
    gameTypeSelect.appendChild(randomisedEl);

    for (i = 0; i < GameTypes.length; i++) {
        var gt = GameTypes[i];
        gt.el = document.createElement("option");
        gt.el.textContent = gt.longNameDisplay;
        gameTypeSelect.appendChild(gt.el);
    };

    if (oldSelectedIndex < 0) {
        gameTypeSelect.selectedIndex = gameType + 1;
        // devlog("gameTypeSelect.selectedIndex under 0", gameTypeSelect.selectedIndex);
    } else {
        gameTypeSelect.selectedIndex = oldSelectedIndex;
    };
};
...
function onPageLoaded () {
    ...
    devlog("onPageLoaded", 3, babylonDB);

    if (lastVer != version || lastZipTimes != latestZipTimes) {
        setTimeout(()=>{
            devlog("fuck this, im not waiting, im outta here");
            window.location.reload(true);
        }, 2500);
    ...
    if (0 < missingFeatures) {
        ...
        if (parsedUrl.query.testMap) {
            ...
            devlog(`testMap startGame(), would've played on gameType ${gameType}, forcing to 0...`);
            playerLimit = 1;
            gameType = 0;
            ws = {
                send: function () {}
        ...
        devlog("onPageLoaded", 19);

        plugins.emit("onPageLoadedEnd", {});
    };
};
...
function joinGame(joinGameIdPassed) { //2nd, request made to server to find game. handles cases where game isnt found
    ...
    ws.onopen = function (e) {
        ...
        output.packInt16U(catalog.get8BitItemId(playerAccount.getPrimaryWeapon(), playerAccount.classIdx));
        output.packInt16U(catalog.get8BitItemId(playerAccount.getSecondaryWeapon(), playerAccount.classIdx));
        output.packInt8U(playerAccount.colorIdx);
        output.packInt16U(catalog.get8BitItemId(playerAccount.hatItem, playerAccount.classIdx));
        output.packInt16U(catalog.get8BitItemId(playerAccount.stampItem, playerAccount.classIdx));
        ...
        const extraParams = {};
        if (playerAccount.isLoggedIn) extraParams.session = playerAccount.session;
        plugins.emit("onExtraParams", { extraParams });
        console.log("after onExtraParams emit");
        ...
        */
        ...
        output.packVeryLongString(JSON.stringify(extraParams));
        ...
        if (!freezeFrame)
            if (e.code == Comm.Close.gameNotFound) {
                ...
                gameKey = gameId = 0;
                document.getElementById("joinCode").value = "";
            } else if (e.code == Comm.Close.locked) {
                console.log("game locked");
                openAlertDialog("Game Locked", "This game has been locked to<br>prevent new players joining.", {
                    label: "OK",
                });
        ...
        switch (joinResult) {
            case Comm.Code.gameJoined:
                ...
                devlog({meId, myTeam, gameType, gameId, gameKey, mapIdx, playerLimit, isGameOwner});
                devlog("mgameType", gameType);

                const extraInfString = input.unPackVeryLongString();
                try {
                    const extraInfo = JSON.parse(extraInfString);
                    await plugins.emit("clientOnExtraGameInfo", { extraInfo });
                    console.log("extra info: " + extraInfString);
                    if (extraInfo.customMinMap) {
                        try {
                            minMap = extraInfo.customMinMap;
                            console.log(`rec cust map ${minMap?.name}`);
                        } catch (e) {
                            console.log(e);
                        };
                    };
                    ...
                    console.log(e);
                    console.error(`parsing failed for exinfo string by server: ${extraInfString}`);
                };
...
async function preloadGame () { //legacyshell on top
    //begins the loading of assets before even getting a verdict from the thing

    devlog("preloading in anticipation of a valid game!!");
    gameScene = new BABYLON.Scene(engine);
    gameScene.preloading = true;
    gameScene.preloaded = true;

    //just an idea i had, obviously this doesnt work
    // if (customizer.scene) {
    //     devlog("stealing meshes from customizer!!", customizer.scene.meshesLoaded, customizer.scene.zipsLoaded);
    //     let stolen = 0;
    //     customizer.scene.meshes.forEach(mesh => {
    //         //new mesh method?
    //         const sharedMesh = new BABYLON.Mesh(mesh.name, gameScene);
    //         sharedMesh.geometry = mesh.geometry;
    //         sharedMesh.material = mesh.material;
    //         sharedMesh.position = mesh.position.clone();
    //         sharedMesh.rotation = mesh.rotation.clone();
    //         sharedMesh.scaling = mesh.scaling.clone();
    //         gameScene.addMesh(sharedMesh);
    //         stolen++;

    //         //instance method?
    //         // const instance = mesh.createInstance(mesh.name);
    //         // if (instance) {
    //         //     instance._scene = gameScene;
    //         //     gameScene.addMesh(instance);
    //         //     stolen++;
    //         // };

    //         //clone method?
    //         // const clonedMesh = mesh.clone(mesh.name, null, true);
    //         // if (clonedMesh) {
    //         //     if (mesh.material) {
    //         //         clonedMesh.material = mesh.material.clone(mesh.material.name);
    //         //     };
    //         //     clonedMesh._scene = gameScene;
    //         //     stolen++;
    //         //     gameScene.addMesh(clonedMesh);
    //         // };

    //         //just adding method?
    //         // if (mesh) {
    //         //     stolen++;
    //         //     gameScene.addMesh(mesh);
    //         // };
    //     });
    //     devlog("stole", stolen, "meshes from customizer (troll)");
    //     gameScene.meshesLoaded = [
    //         ...(gameScene.meshesLoaded || []),
    //         ...(customizer.scene.meshesLoaded || []),
    //     ];
    //     gameScene.zipsLoaded = [
    //         ...(gameScene.zipsLoaded || []),
    //         ...(customizer.scene.zipsLoaded || []),
    //     ];
    //     devlog("steal results:", gameScene.meshesLoaded, gameScene.zipsLoaded, gameScene.meshes);
    // };

    //deal with main models
    (async () => {
        // await sleep(5e3); //simulate long ass loading
        loadObjectMeshes(gameScene);
    })();
    ...
    (async () => {
        mapMeshes = [null];
        // await sleep(5e3); //simulate long ass loading
        loadMeshes(gameScene, ["map"], function (mesh) {
            if (mesh.parent) {
...
function startGame () { //3rd, initialises much of the game's variables, creates scene.
    ...
    if (gameScene && gameScene.preloading) {
    } else {
        gameScene = new BABYLON.Scene(engine);
    };

    gameScene.preloading = false;
    ...
    shadowGen = new BABYLON.ShadowGenerator(1024, shadowLight); // * (fancyShadows ? 7 : 1)
    plugins.emit("onShadowGeneratorCreated", { shadowGen });
            ...
            if (!unfocused) {
                if (document.pointerLockElement) hideGameMenu();
                    ...
                    if (escPressed) {
                        ...
                        wsSend(output, "pause");
                        setRespawnTime((betweenRounds && gameOptions.timedGame.spawnDuringInterval) ? 0 : 5);
                        timeout.set(function () {
                            me.removeFromPlay();
                        }, 3e3);
                    };
...
function loadResources(scene, onComplete) { //4th, orchestrator function for materials, sounds, meshes, map
    ...
    Sounds = {};
    // loadMaterials(scene);
    // devlog("loadResources: loadMaterials complete");

    // loadSounds(function () {
    //     devlog("loadResources: loadSounds complete");
    //     soundsLoaded = true;

    //     loadObjectMeshes(scene, function () {
    //         devlog("loadResources: loadObjectMeshes complete");
    //         loadMapMeshes(scene, function () {
    //             devlog("loadResources: loadMapMeshes complete");
    //             onComplete();
    //         });
    //     });
    // });
    ...
    return Promise.all([
        ...
        devlog("loadResources: ALL resources loaded!");
        onComplete();
    });
};
...
function loadMaterials(scene) { //5th, defines materials
    ...
    // wetMaterial (used for the rain weather effect) moved to the legacyweather plugin's own
    // loadMaterials listener - see the plugins.emit call at the end of this function.
    ...
    mat = new BABYLON.ShaderMaterial("eggShell", scene, "standard", {
        attributes: ["position", "normal", "color", "uv"],
        uniforms: ["world", "view", "viewProjection", "vFogInfos", "vFogColor", "hp", "cameraPosition", "outlineColor", "colorMult", "stampOffset", "uvLowerBound", "uvUpperBound", "sunColor"],
    ...
    mat.onBind = function (mesh) {
        ...
        effect.setFloat2("uvLowerBound", ...uvLowerBound);
        effect.setFloat2("uvUpperBound", ...uvUpperBound);
        effect.setFloat3("sunColor", scene.sunLight.diffuse.r, scene.sunLight.diffuse.g, scene.sunLight.diffuse.b);
    };
    ...
    // Lets a plugin define its own materials right after core's own (e.g. legacyweather's
    // wetMaterial) - loadMaterials had no hook at all before this.
    plugins.emit('loadMaterials', { scene });
};

var soundsLoaded = false;

async function loadSounds(onComplete) { //6th, loads sounds (who knew?)
    if (soundsLoaded) {
        devlog("loadSounds: already loaded");
        onComplete();
        return;
    };
    ...
    //ambiance sound "rain" moved to the legacyweather plugin's own loadSounds listener
    //healing
    ...
    // "thunder" cue registration moved to the legacyweather plugin's own loadSounds listener

    devlog("loadSounds: loading sounds");

    loadSoundsFromList(soundsList, onComplete);
};

function loadObjectMeshes (scene, onComplete) { //7th, loads basically all the things in the models/ folder (name from deobf leak)
    devlog("loading meshes for loadObjectMeshes");

    if (scene.loadObjectMeshes) {
        devlog("already loaded/ing loadObjectMeshes on this scene!");
        setTimeout(() => {
            if (scene.loadObjectMeshes) {
                devlog("doing onComplete for possibly preloaded assets?");
                onComplete && onComplete();
            };
        }, 50);
    } else {
        scene.loadObjectMeshes = true;

        //new
        // loadMeshes(scene, "models.zip?LEGACYSHELLMAPZIPTIMESTAMP", null, onComplete);

        // old
        loadMeshes(scene, loadObjectMeshesBabylons, null, onComplete);

        scene.loadObjectMeshesComplete = true;
    }
};
...
function loadMap(onComplete) { //10th, (name from deobf leak)
    ...
    createMapCells(async function () {
        showSpinner("Building Map", "Turning on the lights...");

        await plugins.emit('createMapCellsMapLoaded', { minMap, SPS, gameScene, shadowGen, shadowLight });
...
async function createMapCells(callback) { //11th
    devlog(minMap);
    minMap = minMap || (playOffline ? JSON.parse(localStorage.getItem("mapBackup")) : (console.log("MAP IDX: " + mapIdx), minMaps[mapIdx]));
    const event = { modify: false, minMap: minMap };
    await plugins.emit('modifyMinMap', event);
    if (event.modify && event.minMap) minMap = event.minMap;
    ...
    await plugins.emit('createMapCells', { minMap, SPS, gameScene, shadowGen, shadowLight });
...
//12th is defined further below from the import

function addMeshToMapHelper(mesh, meshArr) { //13th
    var i = 0;

    // devlog(mesh, "meshArr:", meshArr);

    SPS.addShape(mesh, meshArr.length, {
        positionFunction: function (particle, idx, shapeIdx) { //wait, is it just me or is the map just a bunch of particles? what the heck?
            var rx = meshArr[i].rx || 0;
            var ry = meshArr[i].ry || 0;
            var rz = meshArr[i].rz || 0;
            particle.position.x = meshArr[i].x + .5;
            particle.position.y = meshArr[i].y + .5;
            particle.position.z = meshArr[i].z + .5;
...
function addMeshToMap(keyIdx) { //14th, adds all the shapes to the map and stuff
    ...
    if (illegalMeshes.includes(meshName)) {
        devlog("addMeshToMap skipping illegal:", meshName);
        doStuff = false;
    };
    ...
    if (doStuff) {
        ...
        if ("SPECIAL" != mesh.theme) {
            ...
            addMeshToMapHelper(mesh, meshArr);
        };
    };
...
function generateLightmapAndSkybox(This, complete) { //15th, creates the lightmap (offloaded onto a worker) and skybox
    requestAnimationFrame(function (onComplete) {
        ...
        iterateXYZ(map.width + 1, map.height + 1, map.depth + 1, null, function (lmx, lmy, lmz) {
            ...
            0 < hits && (lightMap[lmx][lmy][lmz] = .2 * brightnessFactor);
        });
...
function onMapComplete() { //16th,  (callback of loadMap at end of onLoadingComplete)
    document.getElementById("serverAndMapInfo").innerHTML = "Server: " + servers[selectedServer].nameDisplay + "<br>Map: " + minMap.name + "<br>Gamemode: " + GameTypes[gameType].longNameDisplay;
    try {
        plugins.emit('onMapCompleteBeforeSetup', { gameScene, minMap });
        var output;
        ...
        // rain/snow particle system setup moved to the legacyweather plugin's own onMapComplete
        // listener (the plugins.emit('onMapComplete', { gameScene }) a bit further up already
        // covers this - no new hook needed).
        ...
        if (playOffline) {
            ...
            //&nbsp;FPS
            players_by_id[meId].hp = 100;
            players_by_id[meId].isGameOwner = true;
            isGameOwner = true;
            doFirstPersonCamera();
            startRendering();
            addCanvasListeners();
...
function startRendering() { //17th, starts rendering and game loop
  hideDarkOverlay();
...
function mainGameLoop () { //18th, this is the final (in a way) stage of the sequence.
    ...
    (async function () {
        ...
        if (++fpsSamples % 10 == 0) {
            ...
            if (timedGame) {
                ...
                } else if (gameOptions.timedGame.spawnDuringInterval) {
                    var date = new Date(((NextRoundTimeout * 1e3) - (Date.now() - roundEndTime)));
                    if (0 <= date) {
                        var minutes = date.getMinutes();
                        ...
                        clockEl.innerText = "Waiting for next game " + minutes + ":" + (seconds < 10 ? "0" : "") + seconds
                    };
                };
            };
        };
        ...
        snowParticleSystem.emitter = me.actor.mesh.position.clone();
        ...
        async function updatePlayers(delta, lerpTick) {
            if (freezeFrame) {
                if (!lerpTick) await me.update(1);
                ...
                await iteratePlayersAsync(async player => {
                    ...
                    if (player.playing) {
                        if (!lerpTick) await player.update(1);
        ...
        if (!(lastTimeStamp < currentTimeStamp)) { //fps exceeds ingame tickrate
            ...
            await updatePlayers(tickRemaining, true);
            ...
            while (lastTimeStamp < currentTimeStamp) {
                ...
                await updatePlayers(tickRemaining);
                tickRemaining = 1;
...
function addPlayer(playerData) {
    devlog("addPlayer FUNCTION", playerData.id == meId, playerData.id, meId, playerData.name, playerData);
    playerData.id == meId && (viewingPlayerId = playerData.id);
    var player = new Player(playerData, gameScene);
    player.id == meId && (viewingPlayer = me = player, me.ws = ws, updateAmmoUi()), player.playing || player.actor.removeFromPlay(), players_by_id[playerData.id] = player, rebuildPlayerList()
};
...
function showMainMenu() {
    ...
    if (inGame) {
        document.getElementById("menuAdContainer").appendChild(document.getElementById("bannerAdContainer"));
        // document.getElementById("inGameAd").style.display = "none;";
        if (me) {
            playerAccount.kills += kills;
            playerAccount.deaths += deaths;
            playerAccount.streak = Math.max(bestOverallStreak, playerAccount.streak);
        ...
        intervalGame.clearAll();
        timeoutGame.clearAll();
        ...
        gameScene.dispose();
        inGame = false;
        if (!customizer) customizer = new Customizer(function () {
            document.getElementById("mainMenu").style.display = "block";
        });
        else {
            console.log("customizer already exists IDIOT!!!");
            document.getElementById("mainMenu").style.display = "block";
    ...
    devlog("showMainMenu set gameType to", gameType);
};
...
function hideGameMenu() {
    if (me) {
        ...
        hideGameSummary();
        if (!(timedGame && (gameOptions.timedGame.spawnDuringInterval ? false : betweenRounds))) {
            document.getElementById("deathBox").style.display = "none";
...
function refreshMapPool () {
    ...
    gameType = document.getElementById("gameTypeSelect").selectedIndex - 1;
    devlog("refreshMapPool set gameType to", gameType);
...
function rebuildPlayerList() { //by player list we mean the literal html element. pretty sure this is purely just for displaying the player list (the one on the left side)
    ...
    if (gameOptions.teamsEnabled) {
        ...
        0 < teams.leader && (lastLeadingTeam = teams.leader), teams.score[lastLeadingTeam] += 1e5, playerIdxs.sort(function (a, b) {
            return players_by_id[b].score + teams.score[players_by_id[b].team] - (players_by_id[a].score + teams.score[players_by_id[a].team])
        })
    } else {
        playerIdxs.sort(function (a, b) {
            return players_by_id[b].score - players_by_id[a].score;
        });
    };

    var list = document.getElementById("playerList").children;
    for (i = 0; i < playerIdxs.length; i++) {
        var player = players_by_id[playerIdxs[i]];
        ...
        list[i].addEventListener("click", function (playerId, uniqueId, username) {
            return function () {
                openAlertDialog(players_by_id[playerId].name, "", {
                    label: "Close",
                ...
                if (playerId != meId) {
                    ...
                    if (players_by_id[playerId].muted) {
...
function addKillText(byPlayer, killedPlayer) {
    ...
    plugins.emit("addKillTextBefore", {msgs})

    var str = '<span style="color: ' + teamColors.text[byPlayer.team] + '">' + byPlayer.name + "</span>" + msgs[Math.randomInt(0, msgs.length)] + '<span style="color: ' + teamColors.text[killedPlayer.team] + '">' + killedPlayer.name + "</span>";

    if ((killEl.innerHTML.match(/<br>/g) || []).length > 4) {
        killEl.innerHTML =
            killEl.innerHTML.substr(
                killEl.innerHTML.search("<br>") + 4
            );
    }

    killEl.innerHTML += str + "<br>";
};

function updatePlayerStatsDisplay() {
    var ratio = Math.floor(playerAccount.kills / Math.max(playerAccount.deaths, 1) * 100) / 100;
    document.getElementById("playerStatValues").innerHTML = (playerAccount.kills || 0).toLocaleString() + "<br>" + (playerAccount.deaths || 0).toLocaleString() + "<br>" + ratio + "<br>" + (playerAccount.streak || 0).toLocaleString()
};

function updateCurrentBalance() {
    document.getElementById("currentBalanceContainer").style.display = "inline-block";
    document.getElementById("currentBalance").innerText = playerAccount.currentBalance;
    plugins.emit("onBalanceUpdated", {balance: playerAccount.currentBalance});
};
...
function onResourcesLoaded() {
    ...
    plugins.emit("onResourcesLoadedEnd", {});
};
...
function menuIn(domName, callback) {
    ...
    callback && callback(); //im just thinking now... why tf did they need a callback for THIS? its not exactly async or a long operation
};
...
function notify(msg, timeout) {
    ...
    var anim = 0;
    if (animIn) clearInterval(animIn);
    animIn = setInterval(function () {
        anim++;
        e.style.opacity = anim / 8;
        e.style.top = anim / 2 - 3.5 + "em";
        if (8 == anim) {
            clearInterval(animIn);
            if (timeout) {
                if (animOut) clearTimeout(animOut);
...
const autocomplete = {
    ...
    show: function (text) {
        ...
        var cmd;
        var mentionPlayers = true;
        var mentionGroups = true;
        if (isCmd && autocomplete.parts.length >= 3) {
            cmd = perm.searchCmdByCategory(autocomplete.parts[0], autocomplete.parts[1]);
            mentionPlayers = cmd.mentionTypes.player;
            mentionGroups = cmd.mentionTypes.group;
            devlog(cmd, mentionPlayers, mentionPlayers);
        };
        ...
        if (autocomplete.commands) {
            autocomplete.suggestions = autocomplete.commands.filter(cmd => {
                return (cmd.replace("@","").toLowerCase()).includes(autocomplete.currentPart.replace("@","").toLowerCase())
            });
        };
        ...
        switch (key) {
            ...
            case "Enter":
                if ("" != text) {
                    ...
                    var output = new Comm.Out(3 + 2 * text.length);
...
function prepareGameSummary() {
    ...
    iteratePlayers(player => {
        playerIdxs[player.team].push(player);
    });

    for (var team = 0; team < 3; team++) {
        teamSummaryEls[team].style.display = "none", playerIdxs[team].sort(function (a, b) {
            var diff = players_by_id[b].score - players_by_id[a].score;
            return 0 == diff && 0 == (diff = players_by_id[a].kills - players_by_id[b].kills) && (diff = players_by_id[a].deaths - players_by_id[b].deaths), diff
        });
        var rowsEl = teamSummaryEls[team];
        for (i = 0; i < playerIdxs[team].length; i++) {
            var player = players_by_id[playerIdxs[team][i]];
            if (player) {
...
function disableShadows() {
    gameScene.shadowsEnabled = false;
    setParticleSystems();
    mapMesh && inGame && (mapMesh.material = gameScene.getMaterialByName("mapNoShadow"));
    return true;
};
...
function bootPlayer(playerId, username) {
    var hasBootPerms = perm.searchPermission("boot", me);
    if (hasBootPerms && players_by_id[playerId] && players_by_id[playerId].username == username) {
        var output = new Comm.Out(2);
        output.packInt8U(Comm.Code.bootPlayer);
        output.packInt8U(playerId);
        // output.packInt16(players_by_id[playerId].username);
...
function fetchServicesInfo(viaButton) {
    var url = parsedUrl.root;
    if (viaButton) {
        notify("Refreshing player counts...", 3e3);
    };
    getRequest(url + "servicesInfo.json?" + Date.now(), function (err, res) {
        if (!err) {
            servicesInfo = JSON.parse(res);
            // Object.keys(servicesInfo.gameInfo).forEach(function (k) {
            //     k = Number(k);
            //     var info = servicesInfo.gameInfo[k];
            //     console.log(k, info);
            // });
            for (const server of servers) {
                server.info = servicesInfo.gameInfo[server.id];
            };
            selectServer(selectedServer);

            if (servicesInfo?.announcement && servicesInfo.announcement !== "") {
                document.getElementById("announcement").textContent = servicesInfo.announcement;
...
function shellFragBurst(player, count, theSize, theAnimLength) {
    plugins.emit("shellFragBurstBefore", { player, count, theSize, theAnimLength });
    if (plugins.cancel) return;
    for (var i = 0; i < count; i++) {
        var animLength = Math.random() * theAnimLength + .5 * theAnimLength * Math.max(1, player.modifiers.scale);
        var size = (i / (.5 * count) * theSize + .5 * theSize) * player.modifiers.scale;
        var se = .08 / (size + 1);
        var dx = (2 * Math.random() - 1) * se * player.modifiers.scale;
        var dy = (2 * Math.random() - .5) * se * player.modifiers.scale;
        var dz = (2 * Math.random() - 1) * se * player.modifiers.scale;
...
function yolkBurst(player) {
    for (var i = 0; i < 10; i++) {
        var animLength = 80 * Math.random() + 80,
            size = (i / 5 * .2 + .2) * player.modifiers.scale,
            se = .05 / (size + 1),
            dx = (2 * Math.random() - 1) * se,
        ...
        s.easing = Ease.inQuad;
        s.position.x = player.actor.mesh.position.x;
        s.position.y = player.actor.mesh.position.y + (.3 * player.modifiers.scale);
        s.position.z = player.actor.mesh.position.z;
        s.startSize = size;
        s.endSize = 0;
        s.gravity = .005;
...
function loadSettings() {
    ...
    settings.fastPollMouse = getStoredBool("fastPollMouse", false);
    ...
    for (var p in inputToControlMap) {
        ...
        document.getElementById("fastPollMouse").checked = settings.fastPollMouse;
...
function selectServer(i) {
    ...
    for (const server of servers) {
        var count = "";
        if (server.info) {
            count = `[${server.info.playerCountTotal.both}] `;
        };
        server.nameDisplay = `${count}${server.name}`;
    };

    document.getElementById("serverName").innerText = "Server: " + servers[selectedServer].nameDisplay;

    serverInfo = servicesInfo.gameInfo[i];

    GameTypes.forEach(gameType => {
        if (serverInfo) {
            gameType.shortNameDisplay = `[${serverInfo.playerCountPublic[gameType.value]}] ${gameType.shortName}`;
            gameType.longNameDisplay = `[${serverInfo.playerCountPublic[gameType.value]}] ${gameType.longName}`;
        };
    });
    ...
    me.playing && (document.pointerLockElement || unfocused) && function (control) {
        switch (control) {
            ...
            case "right":
                ...
                break;
            case "fire":
                document.pointerLockElement && (!betweenRounds) && me && me.pullTrigger();
            ...
            case "jump":
                ...
                break;
            case "grenade":
                document.pointerLockElement && (!betweenRounds) && me && !grenadePowerUp && me.canSwapOrReload() && 0 < me.grenadeCount && (grenadePowerUp = true, grenadeThrowPower = -.15);
                break;
            case "scope":
                settings.holdToAim ? me.actor.scopeIn() : me.actor.scope ? me.actor.scopeOut() : me.actor.scopeIn();
                break;
            case "reload":
                (!betweenRounds) && me.reload();
                break;
            case "weapon":
                (!betweenRounds) && me.swapWeapon(0 == me.weaponIdx ? 1 : 0);
                break;
            case "unfocus":
                unfocused = !unfocused;
                if (unfocused) {
...
function onKeyDown(e) {
    ...
    if (key != lastKey) {
        ...
        if ((grenadePowerUp || 0 != me.controlKeys || ("Enter" != key && "/" != key && "@" != key) || !settings.enableChat) && !keysReleased) {
            ...
            if(k=="4" && playOffline){
              if(players_by_id[meId]){
                const pl = players_by_id[meId];
                pl.x = mapTest.x;
                pl.y = mapTest.y;
                pl.z = mapTest.z;
                pl.pitch = mapTest.pitch;
                pl.yaw = mapTest.yaw;
              }
            }
            if (" " == k && (k = "SPACE", e.preventDefault()), debug) {
                ...
                if ("\\" == k) {
                    ...
                    iteratePlayers(player => {
                        player.actor?.mesh.setVisible(true);
                        player.actor?.showNameSprite();
                        player.actor?.positionNameSprite();
                    });
                    ws.close()
                }
            }
...
class PlayerAccountConstructor { //converted to class just because all these wannabe classes piss me off
    ...
    loggedIn (accountData) {
        ...
        sessionExpiredTimeout = setTimeout(() => {
            openAlertDialog("Session Expired", "Your current session has expired.<br>Please refresh the page to continue using your account's features.", {
                label: "Refresh",
                width: "6em",
                onclick: reloadPage
            }, {
        ...
        }, distributed_data.session_expiry_time * 60e3);
    };
    ...
    syncToServer () {
        ...
        servicesWs.onopen = async function (e) {
            await plugins.emit("syncToServer", {loadout})
            servicesWs.send(JSON.stringify(loadout)), console.log("servicesWs opened, and saveEquip request sent")
        }, servicesWs.onmessage = function (e) {
...
function waitForSetupAndAuthComplete() {
    // devlog("waitForSetupAndAuthComplete1", 1);
    plugins.emit("waitForSetupAndAuthComplete", {waitIteration});
    if (waitIteration >= 600) {
        devlog("waitForSetupAndAuthComplete99", 99);
        openAlertDialog("Uh oh, there was a problem.", '<div id="setupAndAuthTimedOut">Try reloading the page.<br>If that doesn\'t work, try clearing data.</div>', {
            label: "Clear Data",
            width: "8em",
            onclick: deleteData
        }, {
        ...
        plugins.emit("waitForSetupAndAuthCompleteCheck", {waitIteration, customizer, authAttemptComplete, houseAdsLoaded});
        // devlog("waitForSetupAndAuthComplete2", 2);
        if (null != customizer && true === customizer.isReady && authAttemptComplete && houseAdsLoaded) {
            plugins.emit("waitForSetupAndAuthCompleteReady", {waitIteration});
            devlog("waitForSetupAndAuthComplete3", 3);
            customizer.setupItems();
            customizer.dressUpEgg();
            customizer.close();
            setupUpgradeUI();
            devlog("waitForSetupAndAuthComplete4", 4);
            pingServers(true, function () {
                var showDate;
                devlog("waitForSetupAndAuthComplete5", 5);
                onResourcesLoaded();
                devlog("waitForSetupAndAuthComplete6", 6);
                isRunning = true;
                (function () {
                    devlog("waitForSetupAndAuthComplete7", 7);
                ...
                }()) || tryShowTaggedItems() || tryShowNugget() || (document.getElementById("menuAdContainer").appendChild(document.getElementById("bannerAdContainer")), hideDarkOverlay(), showSmallAd());
            });
        } else {
            // devlog("waitForSetupAndAuthComplete8", 8, customizer, authAttemptComplete, houseAdsLoaded);
            waitIteration++;
    ...
    return {
        ...
        openServerList: function () {
            ...
            for (var serverItem = document.getElementById("serverItem"), list = document.createElement("form"), barColors = ["lime", "yellow", "orange", "red"], i = 0; i < servers.length; i++) {
                ...
                nameEl.innerText = servers[i].nameDisplay, checkEl.value = i, servers[i].ping ? (border = Math.min(5.4, Math.max(0, (servers[i].ping - 50) / 65)) + .1, pingColor = Math.min(3, Math.floor(servers[i].ping / 100)), barEl.style.display = "block", offlineEl.style.display = "none", barEl.style.backgroundColor = barColors[pingColor], barEl.style.borderRight = border + "em solid var(--egg-brown)", msEl.innerText = servers[i].ping + "ms") : (barEl.style.display = "none", offlineEl.style.display = "block"), list.appendChild(server)
            }
        ...
        selectGameType: function (i) {
            gameType = i - 1;
            devlog("selectGameType set gameType to", gameType);
        ...
        respawn: function () {
            if (playOffline) return me.playing = true, startBGM(), void canvas.requestPointerLock({ unadjustedMovement: settings.fastPollMouse });
        ...
        setCheckOption: function (el) {
            ...
            try {
                switch (el.id) {
                    ...
                    case "autoDetail":
                        try {
                            el.checked ? enableAutoDetail() : (optimizer.stop(), optimizer.dispose(), optimizer = null);
                        } catch (error) {
                            console.error("Error in setCheckOption autoDetail:", error);
                        };
                        setDetailSettingsVisibility(el.checked);
                        break;
                    case "shadowsEnabled":
                ...
                console.error("Error in setCheckOption:", error);
            };
            settings[el.id] = val;
        ...
        enterSpectatorMode: function () {
            if (me) {
                ...
                if (!(timedGame && betweenRounds)) {
                    ...
                    if (chatting) chatInEl.focus();
                }

                hideMultisizeBannerAd();
                // document.getElementById("gameAdContainer").style.display = "none";

                // pick a player that's not me
                let player;
                iteratePlayers(p => {
                    if (p.id !== meId && p.playing) {
                        player = p;
                ...
                // handle viewing player
                if (player.id !== meId) {
                    viewingPlayer.actor.hands.setRenderingGroupId(0);
                    viewingPlayer.weapon.actor.gunMesh.setRenderingGroupId(0);
                    if (viewingPlayer.actor.hat) viewingPlayer.actor.hat.setEnabled(true);
                }

                // handle player actor
                player.actor.hands.setRenderingGroupId(1);
                player.weapon.actor.gunMesh.setRenderingGroupId(1);
                if (player.actor.hat) player.actor.hat.setEnabled(false);
                player.actor.hideNameSprite();

                // set first-person view
                viewingPlayer = player;
                viewingPlayerId = player.id;
                doFirstPersonCamera(player);

                // update UI
                document.getElementById("spectatingName").innerText = player.name;
            }
        },
...
if (pageLoaded && !alreadyLoaded) {
    console.log("invoking the late load fallback");
    onPageLoaded();
};
