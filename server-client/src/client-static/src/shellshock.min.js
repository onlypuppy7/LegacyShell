    ...
    while (input.isMoreDataAvailable()) {
        ...
        switch (cmd) {
            ...
            case Comm.Code.sync:
                ...
                if (id == meId) {
                    for (var i = 0; i < FramesBetweenSyncs; i++) {
                        //is this just discarded? seems like it so that it doesnt mess up the next packets.
                        input.unPackInt8U();
                        input.unPackRadU();
                        input.unPackRad();
                    };
                    ...
                    for (oldKeys; i != player.stateIdx; i = Math.mod(i + 1, stateBufferSize)) {
...
// [LS] Ingame stuff
wsSend = function (output, CommCode, passedWs) {
...
function rebuildPlayerList() { //by player list we mean the html element. pretty sure this is purely just for displaying player list on the left side
    var playerIdxs = [];
    for (var i = 0; i < players.length; i++) {
