...
const LegacyShellOnMessage = function (msg) {
    ...
    while (input.isMoreDataAvailable()) {
        ...
        if (cmd !== Comm.Code.sync) devlog("received cmd: ", Comm.Convert(cmd), msg);
        switch (cmd) {
            ...
            case Comm.Code.switchTeam:
                ...
                if (player) {
                    ...
                    if (id == meId) myTeam = toTeam, setTeamButton();
                };
                break;
...
PlayerActor.prototype.hit = function () {
    this.hitSoundDelay <= 0 && (this.hitSound.setPosition(new BABYLON.Vector3(this.mesh.position.x, this.mesh.position.y, this.mesh.position.z)), this.hitSound.play(), this.hitSoundDelay = 10)
};
...
GunActor.prototype.addSoundEvent = function (animation, frame, sound) {
    ...
    var event = new BABYLON.AnimationEvent(frame, function () {
        // if (this?.playerActor?.mesh?.position) sound.setPosition(new BABYLON.Vector3(this.playerActor.mesh.position.x, this.playerActor.mesh.position.y, this.playerActor.mesh.position.z))
...
GunActor.prototype.dryFire = function () {
    // this.dryFireSound.setPosition(new BABYLON.Vector3(this.playerActor.mesh.position.x, this.playerActor.mesh.position.y, this.playerActor.mesh.position.z));
...
function DozenGaugeActor(gun, meshName) {
    ...
    this.addSoundEvent("reload", 0, open);
    this.addSoundEvent("reload", 80, load);
    this.addSoundEvent("reload", 115, close);
};
...
function joinGame(joinGameId) { //2nd, request made to server to find game. handles cases where game isnt found
    ...
    var joinGameId = joinGameId || document.getElementById("joinCode").value.trim().slice(-7);
    var mapId = 0;
        ...
        list[i].addEventListener("click", function (playerId, uniqueId, username) {
            return function () {
                ...
                if (playerId != meId) {
                    ...
                    var hasBootPerms = perm.cmds.mod.boot.checkPermissions();
                    if (hasBootPerms) {
...
const commands = ["misc", "mod", "room"]; // List of commands
let autocompleteDialogVisible = false;
let autocompleteIndex = -1;

function showAutocompleteDialog(suggestions) {
    clearAutocompleteDialog();

    if (suggestions.length === 0) return;

    const autocompleteDialog = document.createElement('div');
    autocompleteDialog.id = 'autocomplete-dialog';
    autocompleteDialog.style.position = 'absolute';
    autocompleteDialog.style.background = 'white';
    autocompleteDialog.style.border = '1px solid #ccc';
    autocompleteDialog.style.zIndex = '1000';
    autocompleteDialog.style.fontSize = '0.5em';
    autocompleteDialog.style.bottom = '2.2em';
    autocompleteDialog.style.left = '1em';

    // Add suggestions to the dialog
    suggestions.forEach((cmd, index) => {
        const cmdItem = document.createElement('div');
        cmdItem.textContent = cmd;
        cmdItem.style.padding = '0.5em'; // Padding with em units
        cmdItem.style.cursor = 'pointer';

        cmdItem.onclick = () => {
            chatInEl.value = cmd; // Set the chat input to the selected command
            clearAutocompleteDialog(); // Hide the dialog
        };

        cmdItem.onmouseover = () => {
            autocompleteIndex = index; // Update the index on hover
            highlightAutocompleteItem(autocompleteDialog, index);
        };

        autocompleteDialog.appendChild(cmdItem);
    });

    // Attach the dialog to the chat input element's parent container
    chatInEl.parentElement.style.position = 'relative'; // Ensure parent is relative for absolute positioning
    chatInEl.parentElement.appendChild(autocompleteDialog); // Attach directly to chat input container
    autocompleteDialogVisible = true; // Update the visibility flag
}

// Function to clear the autocomplete dialog
function clearAutocompleteDialog() {
    const existingDialog = document.getElementById('autocomplete-dialog');
    if (existingDialog) {
        existingDialog.remove();
        autocompleteDialogVisible = false; // Reset the visibility flag
    }
}

// Function to highlight the selected item in the autocomplete dialog
function highlightAutocompleteItem(dialog, index) {
    Array.from(dialog.children).forEach((item, i) => {
        item.style.backgroundColor = (i === index) ? '#b3e0ff' : 'white'; // Change background color
    });
}
            ...
            case "Enter":
                if ("" != text) {
                    ...
                    if (!text.startsWith("/")) {
                        ...
                        perm.inputCmd(me, text) && wsSend(output, "chat");
                    };
                };
            case "Tab":
                initChatIn();
...
function onChatKeyUp(e) {
    ...
    if (text.startsWith("/")) {
        const suggestions = commands.filter(cmd => cmd.startsWith(text.slice(1))); // Filter commands based on input
        showAutocompleteDialog(suggestions);
    } else {
        clearAutocompleteDialog();
    };
    ...
    if (autocompleteDialogVisible) {
        switch (key) {
            case "ArrowDown":
                autocompleteIndex = (autocompleteIndex + 1) % commands.length;
                highlightAutocompleteItem(document.getElementById('autocomplete-dialog'), autocompleteIndex);
                e.preventDefault();
                break;
            case "ArrowUp":
                autocompleteIndex = (autocompleteIndex - 1 + commands.length) % commands.length;
                highlightAutocompleteItem(document.getElementById('autocomplete-dialog'), autocompleteIndex);
                e.preventDefault();
                break;
            case "Tab":
            case "Enter":
                chatInEl.value = commands[autocompleteIndex];
            case "Escape":
                clearAutocompleteDialog();
                break;
        };
...
function wsSend (output, CommCode, passedWs) {
    ...
    if (passedWs && passedWs.send) {
        if (Comm.Code[CommCode] !== Comm.Code.sync) devlog("WS SEND:", `${CommCode}:`, Comm.Code[CommCode], output);
...
function shellFragBurst(player, count, theSize, theAnimLength) {
    for (var i = 0; i < count; i++) {
        ...
        var size = i / (.5 * count) * theSize + .5 * theSize;
        var se = .08 / (size + 1);
        var dx = (2 * Math.random() - 1) * se;
        var dy = (2 * Math.random() - .5) * se;
        var dz = (2 * Math.random() - 1) * se;
...
function onKeyDown(e) {
    ...
    if (key != lastKey)
        lastKey = key;
        if ((grenadePowerUp || 0 != me.controlKeys || "Enter" != key || !settings.enableChat) && !keysReleased) {
