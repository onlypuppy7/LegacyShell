...
const LegacyShellOnMessage = function (msg) {
    ...
    while (input.isMoreDataAvailable()) {
        ...
        if (cmd !== Comm.Code.sync && cmd !== Comm.Code.ping) devlog("received cmd: ", Comm.Convert(cmd), msg);
        switch (cmd) {
            ...
            case Comm.Code.die:
                ...
                if (killedPlayer) {
                    ...
                    playSoundIndependent("shellBurst", pos,killedPlayer.actor.playbackRate);
                    ...
                    playSoundIndependent("death.scream."+soundIdx, pos, killedPlayer.actor.playbackRate);
            ...
            case Comm.Code.warp:
                ...
                break;
            case Comm.Code.setScale:
                var id = input.unPackInt8U();
                var player = players[id];

                if (player) player.changeScale(input.unPackInt8U() / 10);
            default:
...
GunActor.prototype.dryFire = function () {
    this.gunEmitter.play(this.dryFireSound, this.playerActor.playbackRate);
    return;
...
GunActor.prototype.denyFire = function () {
    ...
    //TODO: add the playerActor checks again
    this.gunEmitter.play(this.dryFireSound, this.playerActor.playbackRate);
    this.gunEmitter.play(this.denyFireSound, this.playerActor.playbackRate);
...
GunActor.prototype.fire = function () {
    ...
    this.gunEmitter.play(this.fireSound, this.playerActor.playbackRate);
...
function DozenGaugeActor(gun, meshName) {
    ...
    this.dryFireSound = "dozenGauge.dryFire";
    /*
...
function CSG1Actor(gun, meshName) {
    ...
    this.dryFireSound = "csg1.dryFire";
/*
    var removeMag = this.gunMesh.attachSound(Sounds.eggk47.removeMag),
        insertMag = this.gunMesh.attachSound(Sounds.eggk47.insertMag),
...
function Cluck9mmActor(gun, meshName) {
    ...
    this.dryFireSound = "cluck9mm.fire";
    /*
...
function RPEGGActor(gun, meshName) {
    ...
    this.dryFireSound = "rpegg.dryFire";
...
function loadSounds(scene, onComplete) { //6th, loads sounds (who knew?)
    ...
    //function loadSound(src, name, onLoadingComplete) {}; //DELETEME
    //AK
    loadSound("sound/eggk47/fire.mp3", "eggk47.fire", catComplete);
    loadSound("sound/eggk47/dry fire.mp3", "eggk47.dryFire", catComplete);
    loadSound("sound/eggk47/full cycle.mp3", "eggk47.cycle", catComplete);
    loadSound("sound/eggk47/insert mag.mp3", "eggk47.insertMag", catComplete);
    loadSound("sound/eggk47/remove mag.mp3", "eggk47.removeMag", catComplete);
    //dozenGauge
    loadSound("sound/dozenGauge/fire.mp3", "dozenGauge.fire", catComplete);
    loadSound("sound/dozenGauge/open.mp3", "dozenGauge.open", catComplete);
    loadSound("sound/dozenGauge/load.mp3", "dozenGauge.load", catComplete);
    loadSound("sound/dozenGauge/close.mp3", "dozenGauge.close", catComplete);
    //csg1
    loadSound("sound/csg1/fire.mp3", "csg1.fire", catComplete);
    loadSound("sound/csg1/pull action.mp3", "csg1.pullAction", catComplete);
    loadSound("sound/csg1/release action.mp3", "csg1.releaseAction", catComplete);
    //cluck9mm
    loadSound("sound/cluck9mm/fire.mp3", "cluck9mm.fire", catComplete);
    loadSound("sound/cluck9mm/remove mag.mp3", "cluck9mm.removeMag", catComplete);
    loadSound("sound/cluck9mm/insert mag.mp3", "cluck9mm.insertMag", catComplete);
    //rpegg
    loadSound("sound/rpegg/rocketfire.mp3", "rpegg.fire", catComplete);
    loadSound("sound/rpegg/rocketfly.mp3", "rpegg.fly", catComplete);
    loadSound("sound/rpegg/rockethit.mp3", "rpegg.explode", catComplete);
    loadSound("sound/rpegg/rocketload.mp3", "rpegg.load", catComplete);
    loadSound("sound/rpegg/rocketpoof.mp3", "rpegg.poof", catComplete);
    loadSound("sound/rpegg/rocketdeny.mp3", "rpegg.denyFire", catComplete);

    //mis
    loadSound("sound/hammerClick.mp3", "hammerClick", catComplete);
    loadSound("sound/ammo.mp3", "ammo", catComplete);
    loadSound("sound/shellBurst.mp3", "shellBurst", catComplete);
    loadSound("sound/hit.mp3", "hit", catComplete);
    loadSound("sound/rooster.mp3", "endRound", catComplete);

    //grenade
    loadSound("sound/grenade.mp3", "grenade.explode", catComplete);
    loadSound("sound/grenadeBeep.mp3", "grenade.beep", catComplete);
    loadSound("sound/grenadePin.mp3", "grenade.pin", catComplete);

    //deaths
    for (var i = 1; i < 11; i++) {
        loadSound("sound/death/scream" + i + ".mp3", "death.scream." + (i-1) ,catComplete);
    };

    //ambiance (puppy :D)
    loadSound("sound/ambiance/rain.mp3", "rain", catComplete);
    loadSound("sound/ambiance/thunder.mp3", "thunder", catComplete);
    return;
...
function refreshMapPool () {
    ...
    currentMapPool = getMapPool(minMaps, GameTypes[gameType].mapPool);

    for (var i in el.textContent = "[Random Map]", mapSelect.appendChild(el), currentMapPool) {
...
function addChat(str, fromPlayer, chatType) {
    if (fromPlayer) {
        str = '<span style="color: ' + teamColors.text[fromPlayer.team] + '">' + fromPlayer.name + ": </span>" + str;
    };
    ...
    if (chatType === Comm.Chat.cmd) additionalStyles += "; font-style: italic; color: dimgray";
    str = `<div class="shadedPane" style="margin-top: 0.2em${additionalStyles}">` + str + "</div><br>";
    chatOutEl.innerHTML += str
};
...
const autocomplete = {
    ...
    show: function (text) {
        ...
        autocomplete.withoutLast = autocomplete.fulltext.substring(0, Math.max(autocomplete.fulltext.lastIndexOf(" "), 1)) || autocomplete.fulltext;
        ...
        autocomplete.dialog.style.bottom = `${window.innerHeight - rect.top + rect.height - 60.81462}em`;
    ...
    enter: function () {
        chatInEl.value = `${autocomplete.withoutLast}${autocomplete.withoutLast.length <= 1 ? "" : " "}${autocomplete.selected} `;
...
function isMeshVisible(mesh, offset) {
    ...
    var dz = mesh.position.z - camera.globalPosition.zl
...
function wsSend (output, CommCode, passedWs) {
    ...
    if (passedWs && passedWs.send) {
        if (Comm.Code[CommCode] !== Comm.Code.sync && Comm.Code[CommCode] !== Comm.Code.ping) devlog("WS SEND:", `${CommCode}:`, Comm.Code[CommCode], output);
...
function bootPlayer(playerId, uniqueId) {
    var hasBootPerms = perm.searchPermission("boot", me);
    if (hasBootPerms && players[playerId] && players[playerId].uniqueId == uniqueId) {
        var output = new Comm.Out(2);
...
function onKeyDown(e) {
    ...
    if (key != lastKey) {
        lastKey = key;
        if ((grenadePowerUp || 0 != me.controlKeys || ("Enter" != key && "/" != key) || !settings.enableChat) && !keysReleased) {
