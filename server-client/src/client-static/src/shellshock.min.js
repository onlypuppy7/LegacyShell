...
const autocomplete = {
    ...
    show: function (text) {
        ...
        if (autocomplete.parts.length === 1) {
            ...
            var cmdCat = perm.cmds[autocomplete.parts[0]];
            var cmd = cmdCat ? cmdCat[autocomplete.parts[1]] : null;

            console.log("cmd", cmd, autocomplete.parts);
            if (cmd) {
    ...
    showInfo(cmd) {
        autocomplete.commandDialog.innerHTML = `
            <div class="command-info">
                ...
                <p>Usage: ${cmd.usage}</p>
                <p class="command-example">/${cmd.category} ${cmd.identifier} ${cmd.example}</p>
            </div>
        `;
