...
export default async function run () {
    ...
    async function startServer() {
        try {
            ...
            // llmstxt.org expects these at the true site root, not under /wiki/ (where the rest of
            // the wiki is mounted, further down) - served explicitly here, before the closed-mode
            // gate, so they resolve the same way regardless of maintenance mode (same as /discord
            // above). The files themselves are generated by src/scripts/gen-wiki-reference.js - see
            // wiki/docs/06-Contributing/generators.md.
            //
            // Deliberately fs.readFileSync + res.send, not res.sendFile: the path runs through
            // wiki/.vuepress/, and Express's sendFile (via the `send` package) silently 404s on any
            // path containing a dotfile/dot-directory segment by default (a security convention,
            // not a bug) - confirmed by testing res.sendFile against this exact path before
            // switching approaches.
            app.get('/llms.txt', (req, res) => {
                res.type('text/plain').send(fs.readFileSync(path.join(ss.rootDir, 'wiki', '.vuepress', 'public', 'llms.txt'), 'utf8'));
            });
            app.get('/llms-full.txt', (req, res) => {
                res.type('text/plain').send(fs.readFileSync(path.join(ss.rootDir, 'wiki', '.vuepress', 'public', 'llms-full.txt'), 'utf8'));
            });
